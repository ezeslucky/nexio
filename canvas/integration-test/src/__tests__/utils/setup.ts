...
import type { ExtensionType, Store, Transformer } from '@canvas/store';
import { Schema, Text } from '@canvas/store';
import {
  createAutoIncrementIdGenerator,
  TestWorkspace,
} from '@canvas/store/test';

import { effects } from '../../effects.js';
import { TestNexioEditorContainer } from '../../index.js';
...
async function createEditor(
  ...
  const editor = new TestNexioEditorContainer();
  editor.doc = doc;
  editor.mode = mode;
  editor.pageSpecs = [
...
declare global {
  const editor: TestNexioEditorContainer;
  const doc: Store;
