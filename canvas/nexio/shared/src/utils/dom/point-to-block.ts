import { NoteBlockModel, RootBlockModel } from '@canvas/nexio-model';
import { clamp, type Point, type Rect } from '@canvas/global/gfx';
import { BLOCK_ID_ATTR, type BlockComponent } from '@canvas/std';
import { SurfaceBlockModel } from '@canvas/std/gfx';
...
// margin-top: calc(var(--nexio-paragraph-space) + 24px);
...
function isImage({ tagName }: Element) {
  return tagName === 'NEXIO-IMAGE';
}

function isDatabase({ tagName }: Element) {
  return tagName === 'NEXIO-DATABASE';
}
 ...
 * In Chrome/Safari, `document.elementsFromPoint` does not include `nexio-image`.
 */
