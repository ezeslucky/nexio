import { DatabaseBlockModel, ListBlockModel } from '@canvas/nexio-model';
import { type Point, Rect } from '@canvas/global/gfx';
...
export function calcDropTarget(
  ...
  const schema = model.store.schema.get('nexio:database');
  ...
  if (!shouldAppendToDatabase && !matchModels(model, [DatabaseBlockModel])) {
    const databaseBlockComponent =
      element.closest<BlockComponent>('nexio-database');
