...
import type { Point } from '@canvas/global/gfx';
import { BLOCK_ID_ATTR } from '@canvas/std';
...
export function getDropRectByPoint(
  ...
  if (isDatabase) {
    ...
    for (; i < len; i++) {
      ...
      if (e.classList.contains('nexio-database-block-row-cell-content')) {
      ...
      if (e.classList.contains('nexio-database-block-row')) {
  ...
  if (parent?.classList.contains('nexio-database-block-row-cell-content')) {
    result.flag = DropFlags.Database;
...
function getDatabaseBlockTableElement(element: Element) {
  return element.querySelector('.nexio-database-block-table');
}
...
function getDatabaseBlockColumnHeaderElement(element: Element) {
  return element.querySelector('.nexio-database-column-header');
}
...
function getDatabaseBlockRowsElement(element: Element) {
  return element.querySelector('.nexio-database-block-rows');
}

function getCellRect(element: Element, bounds?: DOMRect) {
  if (!bounds) {
    const table = element.closest('.nexio-database-block-table');
    if (!table) {
      return null;
    }
    bounds = table.getBoundingClientRect();
  }
  // nexio-database-block-row-cell
