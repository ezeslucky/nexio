...
import type { DeltaInsert, ExtensionType } from '@canvas/store';
import type { PhrasingContent } from 'mdast';

import type { NexioTextAttributes } from '../../types/index.js';
import {
...
> {
  ...
  astToDelta(ast: MarkdownAST): DeltaInsert<NexioTextAttributes>[] {
    const context = {
      configs: this.configs,
  ...
  deltaToAST(
    deltas: DeltaInsert<NexioTextAttributes>[],
    depth = 0
  ): PhrasingContent[] {
    if (depth > 0) {
