...
class PasteTr {
  ...
  focusPasted = () => {
    ...
    const cursorBlock =
      this.pointState.model.flavour === 'nexio:code' || !this.lastSnapshot
  ...
  constructor(
    ...
    this.firstSnapshotIsPlainText =
      this.firstSnapshot.flavour === 'nexio:paragraph' &&
  ...
  merge() {
    ...
    if (this.pointState.model.flavour === 'nexio:code') {
      this._mergeCode();
      return;
    }
...
function flatNote(snapshot: SliceSnapshot) {
  if (snapshot.content[0]?.flavour === 'nexio:note') {
