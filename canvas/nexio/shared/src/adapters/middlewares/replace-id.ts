  ...
  ({ slots, docCRUD, assetsManager }) => {
    ...
    const replaceSurfaceRefIdSubscription = afterImportBlock$
      ...
      .subscribe(model => {
        ...
        // Otherwise,
        // 1. If the reference is an nexio:frame not in doc, generate a new id.
        // 2. If the reference is graph, keep the original id.
        if (idMap.has(original)) {
          model.props.reference = idMap.get(original)!;
        } else if (
          model.props.refFlavour === 'nexio:frame' &&
    ...
    const beforeImportBlockSubscription = slots.beforeImport
      ...
      .subscribe(payload => {
        ...
        if (snapshot.flavour === 'nexio:page') {
          const index = snapshot.children.findIndex(
            c => c.flavour === 'nexio:surface'
          );
          if (index !== -1) {
        ...
        // Should be re-paired.
        if (['nexio:attachment', 'nexio:image'].includes(snapshot.flavour)) {
        ...
        if (snapshot.flavour === 'nexio:surface') {
