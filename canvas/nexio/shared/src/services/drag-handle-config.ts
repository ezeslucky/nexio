...
import { Extension, Slice, type SliceSnapshot } from '@canvas/store';

export const DndApiExtensionIdentifier = createIdentifier<DNDAPIExtension>(
  'NexioDndApiIdentifier'
);
...
export class DNDAPIExtension extends Extension {
  ...
  fromEntity(options: {
    ...
    const { docId, flavour = 'nexio:embed-linked-doc', blockId } = options;
    ...
    const props = {
      ...
      ...(blockId ? { blockId } : {}),
      pageId: docId,
      style: flavour === 'nexio:embed-synced-doc' ? 'syncedDoc' : 'vertical',
    };
    return {
      ...snapshot,
      content: [
