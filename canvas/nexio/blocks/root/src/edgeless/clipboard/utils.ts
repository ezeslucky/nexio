...
import type { FrameBlockProps } from '@canvas/nexio-model';
import { encodeClipboardBlobs } from '@canvas/nexio-shared/adapters';
import { Bound, getBoundWithRotation } from '@canvas/global/gfx';
...
import { type BlockSnapshot, BlockSnapshotSchema } from '@canvas/store';
...
export function createNewPresentationIndexes(
  ...
  const frames = raw
    .filter((block): block is FrameSnapshot => {
      ...
      return data?.flavour === 'nexio:frame';
    })
