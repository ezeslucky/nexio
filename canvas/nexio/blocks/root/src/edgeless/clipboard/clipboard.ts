import { addAttachments } from '@canvas/nexio-block-attachment';
import { EdgelessFrameManagerIdentifier } from '@canvas/nexio-block-frame';
import { addImages } from '@canvas/nexio-block-image';
import {
  CanvasRenderer,
  DefaultTool,
...
import { splitIntoLines } from '@canvas/nexio-gfx-text';
import type {
  EmbedCardStyle,
  ShapeElementModel,
...
import { type BlockSnapshot, type SliceSnapshot } from '@canvas/store';
...
export class EdgelessClipboardController extends PageClipboard {
  static override key = 'nexio-edgeless-clipboard';
  ...
  private readonly _onPaste = async (_context: UIEventStateContext) => {
    ...
    if (isUrlInClipboard(data)) {
      ...
      // try to interpret url as nexio doc url
  ...
  private async _edgelessToCanvas(
    ...
    const html2canvasOption = {
      ...
      onclone: async function (documentClone: Document, element: HTMLElement) {
        ...
        const layer = documentClone.querySelector('.nexio-edgeless-layer');
  ...
  private async _pasteTextContentAsNote(content: BlockSnapshot[] | string) {
    ...
    if (typeof content === 'string') {
      ...
      for (let index = 0; index < content.length; index++) {
        ...
        if (blockSnapshot.flavour === 'nexio:note') {
