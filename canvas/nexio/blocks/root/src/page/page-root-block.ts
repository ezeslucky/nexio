import { appendParagraphCommand } from '@canvas/nexio-block-paragraph';
import {
  CodeBlockModel,
  ListBlockModel,
  NoteBlockModel,
...
import { Point } from '@canvas/global/gfx';
import type { PointerEventState } from '@canvas/std';
import { BlockComponent, BlockSelection, TextSelection } from '@canvas/std';
import type { BlockModel, Text } from '@canvas/store';
...
export class PageRootBlockComponent extends BlockComponent<RootBlockModel> {
  static override styles = css`
    editor-host:has(> nexio-page-root, * > nexio-page-root) {
      display: block;
      height: 100%;
    }
    ...
    .nexio-page-root-block-container {
      ...
      font-weight: 400;
      max-width: var(--nexio-editor-width);
      margin: 0 auto;

      /* Leave a place for drag-handle */
      /* Do not use prettier format this style, or it will be broken */
      /* prettier-ignore */
      padding-left: var(--nexio-editor-side-padding, ${DOC_BLOCK_CHILD_PADDING}px);
      /* prettier-ignore */
      padding-right: var(--nexio-editor-side-padding, ${DOC_BLOCK_CHILD_PADDING}px);
      /* prettier-ignore */
      padding-bottom: var(--nexio-editor-bottom-padding, ${DOC_BOTTOM_PADDING}px);
    }
  ...
  private _createDefaultNoteBlock() {
    ...
    const noteId = store.addBlock('nexio:note', {}, store.root?.id);
  ...
  private _getDefaultNoteBlock() {
    return (
      this.store.root?.children.find(
        child => child.flavour === 'nexio:note'
  ...
  override connectedCallback() {
    ...
    this.bindHotKey({
      ...
      ArrowUp: () => {
        ...
        if (!prevNote || prevNote.flavour !== 'nexio:note') {
        ...
        const notes = this.store.getModelsByFlavour('nexio:note');
  ...
  override renderBlock() {
    ...
    return html`
      <div class="nexio-page-root-block-container">${children} ${widgets}</div>
    `;
  }

  @query('.nexio-page-root-block-container')
