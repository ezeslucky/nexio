...
import type { BlockSnapshot, DeltaInsert } from '@canvas/store';
...
function toggleStyle(
  inlineEditor: NexioInlineEditor | null,
  attrs: NexioTextAttributes
): void {
  ...
  let oldAttributes: NexioTextAttributes = {};
...
export class RichTextCell extends BaseCellRenderer<Text, string> {
  ...
  get topContenteditableElement() {
    const databaseBlock =
      this.closest<DatabaseBlockComponent>('nexio-database');
  ...
  insertDelta = (delta: DeltaInsert<NexioTextAttributes>) => {
    const inlineEditor = this.inlineEditor$.value;
...
declare global {
  interface HTMLElementTagNameMap {
    'nexio-database-rich-text-cell': RichTextCell;
  }
}
