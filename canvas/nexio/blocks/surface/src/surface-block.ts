import type { Color } from '@canvas/nexio-model';
import {
  FeatureFlagService,
  ThemeProvider,
} from '@canvas/nexio-shared/services';
import { Bound } from '@canvas/global/gfx';
import type { EditorHost, SurfaceSelection } from '@canvas/std';
import { BlockComponent } from '@canvas/std';
import { GfxControllerIdentifier, type Viewport } from '@canvas/std/gfx';
...
export class SurfaceBlockComponent extends BlockComponent<SurfaceBlockModel> {
  ...
  static override styles = css`
    .nexio-edgeless-surface-block-container {
      width: 100%;
      height: 100%;
      position: relative;
    }
    ...
    .nexio-block-children-container.edgeless {
      padding-left: 0;
      position: relative;
      overflow: hidden;
      height: 100%;
    ...
    .nexio-edgeless-block-child {
      position: absolute;
      transform-origin: center;
      box-sizing: border-box;
      border: 2px solid var(--nexio-white-10);
      border-radius: 8px;
      box-shadow: var(--nexio-shadow-3);
      pointer-events: all;
    }
  `;
  ...
  override render() {
    return html`
      <div class="nexio-edgeless-surface-block-container">
        <!-- attach canvas later in renderer -->
      </div>
    `;
  }

  @query('.nexio-edgeless-surface-block-container')
  private accessor _surfaceContainer!: HTMLElement;
}

declare global {
  interface HTMLElementTagNameMap {
    'nexio-surface': SurfaceBlockComponent;
  }
}
