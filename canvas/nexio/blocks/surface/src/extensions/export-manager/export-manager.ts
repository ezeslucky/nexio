import { ImageBlockModel, type RootBlockModel } from '@canvas/nexio-model';
import { FetchUtils } from '@canvas/nexio-shared/adapters';
import {
  CANVAS_EXPORT_IGNORE_TAGS,
  DEFAULT_IMAGE_PROXY_ENDPOINT,
} from '@canvas/nexio-shared/consts';
import type { Viewport } from '@canvas/nexio-shared/types';
import {
  isInsidePageEditor,
  matchModels,
...
import type { ExtensionType, Store } from '@canvas/store';
    ...
    const html2canvasOption = {
      ignoreElements: function (element: Element) {
        if (
          ...
          // the close and expand buttons in nexio-doc-meta-data is not needed to be showed
          return true;
        } else {
          return false;
        }
  ...
  private async _html2canvas(
    ...
    const html2canvasOption = {
      ...
      onclone: async (documentClone: Document, element: HTMLElement) => {
        ...
        const layer = element.classList.contains('.nexio-edgeless-layer')
          ? element
          : null;
