import { type SurfaceRefBlockModel } from '@canvas/nexio-model';
import { unsafeCSSVarV2 } from '@canvas/nexio-shared/theme';
import { BlockComponent, BlockSelection } from '@canvas/std';
...
export class EdgelessSurfaceRefBlockComponent extends BlockComponent<SurfaceRefBlockModel> {
  static override styles = css`
    nexio-edgeless-surface-ref {
      position: relative;
      overflow: hidden;
    }

    .nexio-edgeless-surface-ref-container {
      border-radius: 8px;
      border: 1px solid
    ...
    .nexio-edgeless-surface-ref-container.focused {
  ...
  override renderBlock() {
    return html` <div
      class=${classMap({
        'nexio-edgeless-surface-ref-container': true,
...
declare global {
  interface HTMLElementTagNameMap {
    'nexio-edgeless-surface-ref': EdgelessSurfaceRefBlockComponent;
  }
}
