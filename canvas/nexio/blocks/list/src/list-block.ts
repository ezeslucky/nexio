import '@canvas/nexio-shared/commands';

import { CaptionedBlockComponent } from '@canvas/nexio-components/caption';
import { playCheckAnimation } from '@canvas/nexio-components/icons';
import { TOGGLE_BUTTON_PARENT_CLASS } from '@canvas/nexio-components/toggle-button';
import { DefaultInlineManagerExtension } from '@canvas/nexio-inline-preset';
import type { ListBlockModel } from '@canvas/nexio-model';
...
import { getViewportElement } from '@canvas/nexio-shared/utils';
import type { BlockComponent } from '@canvas/std';
import { BlockSelection, TextSelection } from '@canvas/std';
import {
  getInlineRangeProvider,
  type InlineRangeProvider,
} from '@canvas/std/inline';
import type { BaseSelection } from '@canvas/store';
...
export class ListBlockComponent extends CaptionedBlockComponent<ListBlockModel> {
  ...
  private readonly _onClickIcon = (e: MouseEvent) => {
    ...
    if (this.model.props.type === 'toggle') {
      ...
      if (this.model.props.checked) {
        const checkEl = this.querySelector('.nexio-list-block__todo-prefix');
        if (checkEl) {
  ...
  override renderBlock(): TemplateResult<1> {
    ...
    return html`
      <div class=${'nexio-list-block-container'} style="${textAlignStyle}">
        <div
          class=${classMap({
            'nexio-list-rich-text-wrapper': true,
            'nexio-list--checked':
  ...
  override accessor blockContainerStyles = {
    margin: 'var(--nexio-list-margin, 10px 0)',
  };
}
