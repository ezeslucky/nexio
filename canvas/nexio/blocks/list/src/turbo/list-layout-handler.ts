import type { Rect } from '@canvas/nexio-gfx-turbo-renderer';
import {
...
import type { EditorHost, GfxBlockComponent } from '@canvas/std';
import { clientToModelCoord, type ViewportRecord } from '@canvas/std/gfx';
...
export class ListLayoutHandlerExtension extends BlockLayoutHandlerExtension<ListLayout> {
  ...
  override queryLayout(
    ...
    const listItemSelector =
      '.nexio-list-block-container .nexio-list-rich-text-wrapper [data-v-text="true"]';
    ...
    listItemNodes.forEach(listItemNode => {
      ...
      if (listItemWrapper.classList.contains('nexio-list--checked')) {
        checked = true;
      }
