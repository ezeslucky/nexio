...
} from '@canvas/nexio-inline-reference';
import {
  type AliasInfo,
  type DocMode,
...
import { cloneReferenceInfo } from '@canvas/nexio-shared/utils';
...
import { GfxControllerIdentifier, GfxExtension } from '@canvas/std/gfx';
import { type GetStoreOptions, type Query, Text } from '@canvas/store';
...
export class EmbedSyncedDocBlockComponent extends EmbedBlockComponent<EmbedSyncedDocModel> {
    ...
    class EmbedSyncedDocWatcher extends LifeCycleWatcher {
      ...
      override mounted(): void {
        ...
        view.viewUpdated.subscribe(payload => {
          if (
            payload.type !== 'block' ||
            payload.view.model.flavour !== 'nexio:embed-synced-doc'
          ) {
            return;
          }
  ...
  protected _renderSyncedView = () => {
    ...
    const renderEditor = () => {
      return choose(editorMode, [
        [
          'page',
          () => html`
            <div class="nexio-page-viewport" data-theme=${appTheme}>
              ${new BlockStdScope({
                store: syncedDoc,
        ...
        [
          'edgeless',
          () => html`
            <div class="nexio-edgeless-viewport" data-theme=${edgelessTheme}>
              ${new BlockStdScope({
                store: syncedDoc,
    ...
    return this.renderEmbed(
      () => html`
        <div
          class=${classMap({
            'nexio-embed-synced-doc-container': true,
            [editorMode]: true,
            [theme]: true,
            surface: false,
        ...
        >
          <div class="nexio-embed-synced-doc-editor">
            ${isPageMode && this._isEmptySyncedDoc
              ? html`
                  <div class="nexio-embed-synced-doc-editor-empty">
                    <span>
          ...
          <div
            class=${classMap({
              'nexio-embed-synced-doc-header-wrapper': true,
              selected: this.selected$.value,
            })}
          >
            <div class="nexio-embed-synced-doc-header">
              <span class="nexio-embed-synced-doc-icon"
                >${this.icon$.value}</span
              >
              <span class="nexio-embed-synced-doc-title">${this.title$}</span>
            </div>
          </div>
        </div>
      `
  ...
  @query(
    ':scope > .nexio-block-component > .embed-block-container > nexio-embed-synced-doc-card'
  )
  accessor syncedDocCard: EmbedSyncedDocCard | null = null;

  @query(
    ':scope > .nexio-block-component > .embed-block-container > .nexio-embed-synced-doc-container > .nexio-embed-synced-doc-editor > div > editor-host'
  )
