import { TextUtils } from '@canvas/nexio-block-surface';
import { formatBlockCommand } from '@canvas/nexio-inline-preset';
import {
  DefaultTheme,
...
import { Bound, clamp } from '@canvas/global/gfx';
...
export class EdgelessTextBlockComponent extends GfxBlockComponent<EdgelessTextBlockModel> {
  static override styles = css`
    ...
    .edgeless-text-block-container nexio-paragraph,
    nexio-list {
  ...
  override renderGfxBlock() {
    ...
    const containerStyle: StyleInfo = {
      ...
      border: `1px solid ${editing ? 'var(--nexio—primary—color, #1e96eb)' : 'transparent'}`,
      borderRadius: '4px',
  ...
  override renderPageContent() {
    ...
    return html`
      <div style=${style} class="nexio-block-children-container">
  ...
  tryFocusEnd() {
    const paragraphOrLists = Array.from(
      this.querySelectorAll<BlockComponent>('nexio-paragraph, nexio-list')
    );
  ...
  @query('.nexio-block-children-container')
  accessor childrenContainer!: HTMLDivElement;
}

declare global {
  interface HTMLElementTagNameMap {
    'nexio-edgeless-text': EdgelessTextBlockComponent;
  }
}
              ...
              view.updateComplete
                .then(() => {
                  ...
                  if (model.children.length === 0) {
                    ...
                    const rect = view
                      .querySelector('.nexio-block-children-container')
