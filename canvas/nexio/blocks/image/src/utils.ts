import { autoResizeElementsCommand } from '@canvas/nexio-block-surface';
...
import { BlockSelection, type BlockStdScope } from '@canvas/std';
...
const DEFAULT_ATTACHMENT_NAME = 'nexio-attachment';
...
export async function turnImageIntoCardView(
  ...
  if (!doc.schema.flavourSchemaMap.has('nexio:attachment')) {
  ...
  transformModel(model, 'nexio:attachment', attachmentProp);
}
