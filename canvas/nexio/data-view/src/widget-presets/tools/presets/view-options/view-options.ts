...
const styles = css`
  .nexio-database-toolbar-item.more-action {
    padding: 2px;
    border-radius: 4px;
    display: flex;
    align-items: center;
  ...
  .nexio-database-toolbar-item.more-action:hover {
...
export class DataViewHeaderToolsViewOptions extends WidgetBase {
  ...
  override render() {
    ...
    return html` <div
      class="nexio-database-toolbar-item more-action"
            ...
            const viewTypes = view.manager.viewMetas.map<MenuConfig>(meta => {
              return menu => {
                ...
                const textStyle = styleMap({
                  ...
                  color: isSelected
                    ? 'var(--nexio-text-emphasis-color)'
                    : 'var(--nexio-text-secondary-color)',
                });
                const data: MenuButtonData = {
                  content: () => html`
                    <div
                      style="color:var(--nexio-text-emphasis-color);width:100%;display: flex;flex-direction: column;align-items: center;justify-content: center;padding: 6px 16px;white-space: nowrap"
                    >
                      <div style="${iconStyle}">
