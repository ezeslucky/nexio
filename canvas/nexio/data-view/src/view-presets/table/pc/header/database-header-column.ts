...
export class DatabaseHeaderColumn extends SignalWatcher(
  ...
  static override styles = css`
    nexio-database-header-column {
      display: flex;
    }

    .nexio-database-header-column-grabbing * {
      cursor: grabbing;
    }
  `;
                    ...
                    items: [
                      ...
                      ...numberFormats.map(format => {
                        ...
                        return menu.action({
                          ...
                          prefix: html`<span
                            style="font-size: var(--nexio-font-base); scale: 1.2;"
                            >${format.symbol}</span
                          >`,
                          name: format.label,
                          select: () => {
  ...
  override render() {
    ...
    const classes = classMap({
      'nexio-database-column-move': true,
      [this.grabStatus]: true,
    });
    return html`
      <div
        style=${style}
        class="nexio-database-column-content"
      ...
      >
        ...
        <div class="nexio-database-column-text ${column.type$.value}">
          <div
            class="nexio-database-column-type-icon dv-hover"
            @click="${this._clickTypeIcon}"
          >
            <uni-lit .uni="${column.icon}"></uni-lit>
          </div>
          <div class="nexio-database-column-text-content">
            <div class="nexio-database-column-text-input">
              ${column.name$.value}
            </div>
          </div>
        </div>
...
function numberFormatConfig(column: Property): MenuConfig {
  return () =>
    html` <nexio-database-number-format-bar
      .column="${column}"
    ></nexio-database-number-format-bar>`;
}

declare global {
  interface HTMLElementTagNameMap {
    'nexio-database-header-column': DatabaseHeaderColumn;
  }
}
