...
const styles = css`
  nexio-data-view-table-group:hover .group-header-op {
    visibility: visible;
    opacity: 1;
  }
...
export class TableGroup extends SignalWatcher(
  ...
  private renderRows(rows: Row[]) {
    return html`
      ...
      ></nexio-database-column-header>
      <div class="nexio-database-block-rows">
        ${repeat(
          rows,
          row => row.rowId,
          (row, idx) => {
        ...
        : html` <div
          ...
          >
            <div
              ...
              data-test-id="nexio-database-add-row-button"
              role="button"
            >
              ${PlusIcon()}<span style="font-size: 12px">New Record</span>
            </div>
          </div>`}
      <nexio-database-column-stats
        .tableViewLogic="${this.tableViewLogic}"
        .group="${this.group}"
      >
      </nexio-database-column-stats>
  ...
  @query('.nexio-database-block-rows')
...
declare global {
  interface HTMLElementTagNameMap {
    'nexio-data-view-table-group': TableGroup;
  }
}
