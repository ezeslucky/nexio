...
export class RecordField extends SignalWatcher(
  ...
  static override styles = css`
    ...
    nexio-data-view-record-field .icon {
      display: flex;
      align-items: center;
      width: 16px;
      height: 16px;
    ...
    nexio-data-view-record-field .icon svg {
      width: 16px;
      height: 16px;
    ...
    .field-content .nexio-database-number {
      text-align: left;
  ...
  changeEditing = (editing: boolean) => {
    const selection = this.closest('nexio-data-view-record-detail')?.selection;
    if (selection) {
      selection.selection = {
...
declare global {
  interface HTMLElementTagNameMap {
    'nexio-data-view-record-field': RecordField;
  }
}
