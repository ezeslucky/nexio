...
import type { NoteChildrenFlavour } from '@canvas/nexio-shared/types';
import {
  handleNativeRangeAtPoint,
  hasClassNameInList,
} from '@canvas/nexio-shared/utils';
import { type IPoint, Point, serializeXYWH } from '@canvas/global/gfx';
import type { BlockStdScope, PointerEventState } from '@canvas/std';
import {
  BaseTool,
...
export class NoteTool extends BaseTool<NoteToolOption> {
  static override toolName = 'nexio:note';
  ...
  override activate() {
    const attributes =
      this.std.get(EditPropsStore).lastProps$.value['nexio:note'];
