...
export class RichText extends WithDisposable(ShadowlessElement) {
  ...
  private readonly _inlineEditor$ = signal<NexioInlineEditor | null>(null);
  ...
  private _init() {
    ...
    // init inline editor
    this._inlineEditor$.value = new InlineEditor<NexioTextAttributes>(
      this._yText,
      {
  ...
  accessor embedChecker: <
    TextAttributes extends NexioTextAttributes = NexioTextAttributes,
  >(
  ...
  @property({ attribute: false })
  accessor markdownMatches: InlineMarkdownMatch<NexioTextAttributes>[] = [];
