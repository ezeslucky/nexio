...
export const sideMap = new Map([
  ...
  ['nexio:surface:frame', { top: 28 }],
  // includes group element
  ['nexio:surface:group', { top: 20 }],
  // has only one shape element
  ['nexio:surface:shape', { top: 26, bottom: -26 }],
]);
...
export function autoUpdatePosition(
  ...
  const isInline = flavour === 'nexio:note';
...
export function renderToolbar(
  ...
  const actions = [
    flavour,
    `custom:${flavour}`,
    hasSurfaceScope ? ['nexio:surface:*', 'custom:nexio:surface:*'] : [],
    'nexio:*',
    'custom:nexio:*',
  ]
    .flat()
  ...
  // Avoids shaking
  if (flavour === 'nexio:note' && context.std.range.value) {
