...
import { BlockStdScope, BlockViewIdentifier } from '@canvas/std';
import type {
  BlockModel,
  BlockViewType,
  ExtensionType,
  ...
  getPreviewStd = (blockIds: string[]) => {
    ...
    const extensions = [
      ...
      EditorSettingExtension({ setting$: settingSignal }),
      {
        setup(di) {
          di.override(
            BlockViewIdentifier('nexio:database'),
            () => literal`nexio-dnd-preview-database`
          );
        },
      } as ExtensionType,
      {
        setup(di) {
          di.override(BlockViewIdentifier('nexio:image'), () => {
            return (model: BlockModel) => {
              const parent = model.store.getParent(model.id);

              if (parent?.flavour === 'nexio:surface') {
                return literal`nexio-edgeless-placeholder-preview-image`;
              }

              return literal`nexio-placeholder-preview-image`;
            };
          });
        },
      } as ExtensionType,
    ...
    const noteBlock = this.widget.host.querySelector('nexio-note');
