import { type CalloutBlockComponent } from '@canvas/nexio-block-callout';
import {
  NEXIO_EDGELESS_NOTE,
  EdgelessNoteBackground,
  EdgelessNoteBlockComponent,
} from '@canvas/nexio-block-note';
...
import type { BlockComponent, EditorHost } from '@canvas/std';
import type {
  BaseSelection,
  BlockModel,
  BlockSnapshot,
...
export const insideDatabaseTable = (element: Element) => {
  return !!element.closest('.nexio-database-block-table');
};
...
export const getParentNoteBlock = (blockComponent: BlockComponent) => {
  return blockComponent.closest('nexio-note, nexio-edgeless-note') ?? null;
};
...
export const getClosestNoteBlock = (
  ...
  return isInsidePageEditor
    ? findClosestBlockComponent(rootComponent, point, 'nexio-note')
...
export const getClosestBlockByPoint = (
  ...
  if (!closestNoteBlock || closestNoteBlock.closest('.nexio-surface-ref')) {
    return null;
  }
  ...
  const blockSelector =
    '.nexio-note-block-container > .nexio-block-children-container > [data-block-id]';
  ...
  if (matchModels(closestBlock.model, [ParagraphBlockModel])) {
    const callout =
      closestBlock.closest<CalloutBlockComponent>('nexio-callout');
    if (callout) {
      return callout;
    }
  }
...
function getHoveringNote(point: Point) {
  ...
  for (const el of elements) {
    ...
    if (el instanceof EdgelessNoteBackground) {
      return el.closest(NEXIO_EDGELESS_NOTE) ?? null;
    }
  }
  return null;
}
