import { EdgelessCRUDIdentifier } from '@canvas/nexio-block-surface';
...
import type { IVec, Point, Rect } from '@canvas/global/gfx';
import { type BlockComponent, WidgetComponent } from '@canvas/std';
...
import type { NEXIO_DRAG_HANDLE_WIDGET } from './consts.js';
...
export class NexioDragHandleWidget extends WidgetComponent<RootBlockModel> {
  ...
  override render() {
    ...
    const classes = {
      'nexio-drag-handle-grabber': true,
      dots: isGfx ? true : false,
    };

    return html`
      <div class="nexio-drag-handle-widget">
        <div class="nexio-drag-handle-container">
        ...
        <div class="nexio-drag-hover-rect" style=${hoverRectStyle}></div>
      </div>
    `;
  }

  @query('.nexio-drag-handle-container')
  accessor dragHandleContainer!: HTMLDivElement;

  @query('.nexio-drag-handle-grabber')
...
declare global {
  interface HTMLElementTagNameMap {
    [NEXIO_DRAG_HANDLE_WIDGET]: NexioDragHandleWidget;
  }
}
