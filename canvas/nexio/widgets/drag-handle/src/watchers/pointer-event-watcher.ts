import type { NoteBlockComponent } from '@canvas/nexio-block-note';
import { captureEventTarget } from '@canvas/nexio-shared/utils';
...
import { NEXIO_DRAG_HANDLE_WIDGET } from '../consts.js';
...
export class PointerEventWatcher {
  ...
  private readonly _clickHandler: UIEventHandler = ctx => {
    ...
    const insideDragHandle = !!element?.closest(NEXIO_DRAG_HANDLE_WIDGET);
  ...
  private readonly _pointerOutHandler: UIEventHandler = ctx => {
    ...
    const inPage = !!relatedElement?.closest('.nexio-page-viewport');

    const inDragHandle = !!relatedElement?.closest(NEXIO_DRAG_HANDLE_WIDGET);
  ...
  private readonly _throttledPointerMoveHandler = throttle<UIEventHandler>(
    ctx => {
      ...
      if (element.closest('.nexio-drag-handle-container')) return;
