import { getInlineEditorByModel } from '@canvas/nexio-rich-text';
import type { NexioInlineEditor } from '@canvas/nexio-shared/types';
import { DisposableGroup } from '@canvas/global/disposable';
import type { UIEventStateContext } from '@canvas/std';
import { TextSelection, WidgetComponent } from '@canvas/std';
import { InlineEditor } from '@canvas/std/inline';
import debounce from 'lodash-es/debounce';

import { NEXIO_SLASH_MENU_TRIGGER_KEY } from './consts';
...
export class NexioSlashMenuWidget extends WidgetComponent {
  ...
  private readonly _handleInput = (
    ...
    if (this.block?.model.flavour !== 'nexio:page') {
    ...
    inlineRangeApplyCallback(() => {
      ...
      if (!text.endsWith(NEXIO_SLASH_MENU_TRIGGER_KEY)) return;
  ...
  private readonly _onCompositionEnd = (ctx: UIEventStateContext) => {
    ...
    if (event.data !== NEXIO_SLASH_MENU_TRIGGER_KEY) return;
  ...
  private readonly _onKeyDown = (ctx: UIEventStateContext) => {
    ...
    if (event.isComposing || key !== NEXIO_SLASH_MENU_TRIGGER_KEY) return;
