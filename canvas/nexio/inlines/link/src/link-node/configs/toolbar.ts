...
import { NexioLink } from '../nexio-link';
  ...
  actions: [
    {
      ...
      content(cx) {
        ...
        if (!(target instanceof NexioLink)) return null;

        const { link } = target;
        if (!link) return null;

        return html`<nexio-link-preview .url=${link}></nexio-link-preview>`;
      },
    },
    {
      ...
      actions: [
        {
          ...
          run(ctx) {
            ...
            if (!(target instanceof NexioLink)) return;
    ...
    },
    {
      ...
      actions: [
        ...
        },
        {
          ...
          when(ctx) {
            const target = ctx.message$.peek()?.element;
            if (!(target instanceof NexioLink)) return false;
      ...
      when(ctx) {
        ...
        const bookmarkSchema = schema.flavourSchemaMap.get('nexio:bookmark');
