...
function getCombinedFormatFromInlineEditors(
  inlineEditors: [NexioInlineEditor, InlineRange | null][]
): NexioTextAttributes {
  const formatArr: NexioTextAttributes[] = [];
  ...
  return formatArr.reduce((acc, cur) => {
    const newFormat: NexioTextAttributes = {};
    for (const key in acc) {
      const typedKey = key as keyof NexioTextAttributes;
...
function getSelectedInlineEditors(
  ...
  filter: (
    inlineRoot: InlineRootElement<NexioTextAttributes>
  ) => InlineEditor<NexioTextAttributes> | []
) {
...
function handleCurrentSelection(
  ...
  handler: (
    ...
    inlineEditors: InlineEditor<NexioTextAttributes>[]
  ) => { textAttributes: NexioTextAttributes } | boolean | void
): Chain<InitCommandCtx & { textAttributes: NexioTextAttributes }> {
  return chain.try(chain => [
    ...
    chain.pipe((ctx, next) => {
      const selectedInlineEditors = Array.from<InlineRootElement>(
        ...
        .map((el): NexioInlineEditor => el.inlineEditor);
