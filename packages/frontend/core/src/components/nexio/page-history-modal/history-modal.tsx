...
const PlanPrompt = () => {
      ...
      <div className={styles.planPromptTitle}>
        {
          isProWorkspace !== null
            ? !isProWorkspace
              ? t[
                  'com.nexio.history.confirm-restore-modal.plan-prompt.limited-title'
                ]()
              : t[
                  'com.nexio.history.confirm-restore-modal.plan-prompt.title'
                ]()
  ...
  const planDescription = useMemo(() => {
    if (!isProWorkspace) {
      return (
        <>
          <Trans i18nKey="com.nexio.history.confirm-restore-modal.free-plan-prompt.description">
          ...
          {isOwner ? (
            ...
            >
              {t[
                'com.nexio.history.confirm-restore-modal.pro-plan-prompt.upgrade'
              ]()}
            </span>
          ) : null}
        </>
      ...
      return (
        <Trans i18nKey="com.nexio.history.confirm-restore-modal.pro-plan-prompt.description">
  ...
  return (
    <div className={styles.historyList}>
      <div className={styles.historyListHeader}>
        {t['com.nexio.history.version-history']()}
      </div>
                    ...
                    return (
                      <Fragment key={history.timestamp}>
                        ...
                        {isLastGroup && isLastItem && onLoadMore ? (
                          ...
                          >
                            {t[
                              'com.nexio.history.confirm-restore-modal.load-more'
                            ]()}
                          </Button>
                        ) : null}
                      </Fragment>
  ...
  return (
    ...
    >
      ...
      <div className={styles.emptyHistoryPromptTitle}>
        {t['com.nexio.history.empty-prompt.title']()}
      </div>
      <div className={styles.emptyHistoryPromptDescription}>
        {t['com.nexio.history.empty-prompt.description']()}
      </div>
    </div>
  );
};
...
const PageHistoryManager = ({
  ...
  const onConfirmRestore = useCallback(() => {
    openConfirmModal({
      title: t['com.nexio.history.restore-current-version'](),
      description: t['com.nexio.history.confirm-restore-modal.hint'](),
      cancelText: t['Cancel'](),
      contentOptions: {
        ['data-testid' as string]: 'confirm-restore-history-modal',
        style: { padding: '20px 26px' },
      },
      confirmText: t['com.nexio.history.confirm-restore-modal.restore'](),
    ...
    <div className={styles.root}>
      ...
      <div className={styles.historyFooter}>
        <Button onClick={onClose}>
          {t['com.nexio.history.back-to-page']()}
        </Button>
        ...
        >
          {t['com.nexio.history.restore-current-version']()}
        </Button>
      </div>
    </div>
  );
