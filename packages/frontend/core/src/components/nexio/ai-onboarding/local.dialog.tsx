...
export const AIOnboardingLocal = () => {
  ...
  const actions = useMemo(() => {
    const result: NonNullable<Notification['actions']> = [
      {
        key: 'learn-more',
        label: t['com.nexio.ai-onboarding.local.action-learn-more'](),
        onClick: () => {
          window.open('https://ai.nexio.pro', '_blank', 'noreferrer');
        },
      },
    ];
    if (notSignedIn) {
      result.push({
        key: 'get-started',
        label: t['com.nexio.ai-onboarding.local.action-get-started'](),
        onClick: () => {
      ...
      const id = notify(
        {
          title: (
            <div className={styles.title}>
              {t['com.nexio.ai-onboarding.local.title']()}
            </div>
          ),
          message: t['com.nexio.ai-onboarding.local.message'](),
          icon: <AiIcon />,
