  ...
  public async writeDoc(options: {
    ...
    if (!docId) {
      ...
      if (updateStrategy === 'override') {
        ...
        const pageBlockId = doc.getBlocksByFlavour('nexio:page')[0]?.id;
        const blockId = doc.addBlock('nexio:note', {}, pageBlockId);
        await MarkdownTransformer.importMarkdownToBlock({
          doc,
          blockId,
          markdown: `---\n${markdown}`,
