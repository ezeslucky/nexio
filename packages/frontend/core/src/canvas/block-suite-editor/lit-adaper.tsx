...
import {
  ...
  LitEdgelessEditor,
  type PageEditor,
} from '@nexio/core/canvas/editors';
...
import { useInsidePeekView } from '@nexio/core/modules/peek-view';
...
import { CanvasEditorJournalDocTitle } from './journal-doc-title';
...
export const CanvasDocEditor = forwardRef<
  PageEditor,
  CanvasEditorProps & {
    onClickBlank?: () => void;
    titleRef?: React.Ref<DocTitle>;
  }
>(function CanvasDocEditor(
  {
    page,
    shared,
    onClickBlank,
    ...
    <>
      <div className={styles.nexioDocViewport}>
        ...
        {!isJournal ? (
          ...
          <CanvasEditorJournalDocTitle page={page} />
        )}
...
export const CanvasEdgelessEditor = forwardRef<
  EdgelessEditor,
  CanvasEditorProps
>(function CanvasEdgelessEditor({ page }, ref) {
    ...
    if (editorRef.current) {
      editorRef.current.updateComplete
        .then(() => {
          ...
          editorRef.current
            ?.querySelector<HTMLElement>('nexio-edgeless-root')
            ?.click();
        })
        .catch(console.error);
    }
