...
function actionToStream<T extends keyof CanvasPresets.AIActions>(
  ...
  if (extract && typeof extract === 'function') {
    return (host: EditorHost, ctx: AIContext): CanvasPresets.TextStream => {
  ...
  return (host: EditorHost): CanvasPresets.TextStream => {
    let stream: CanvasPresets.TextStream | undefined;
    return {
      async *[Symbol.asyncIterator]() {
...
function actionToGeneration<T extends keyof CanvasPresets.AIActions>(
  id: T,
  variants?: Omit<
    Parameters<CanvasPresets.AIActions[T]>[0],
    keyof CanvasPresets.AITextActionOptions
...
function updateEdgelessAIPanelConfig<
  T extends keyof CanvasPresets.AIActions,
>(
