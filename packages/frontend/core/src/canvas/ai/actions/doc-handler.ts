...
export function bindTextStream(
  stream: CanvasPresets.TextStream,
  {
    update,
    finish,
    signal,
...
function actionToStream<T extends keyof CanvasPresets.AIActions>(
  host: EditorHost,
  id: T,
  input: string,
  signal?: AbortSignal,
  variants?: Omit<
    Parameters<CanvasPresets.AIActions[T]>[0],
    keyof CanvasPresets.AITextActionOptions
  >,
  trackerOptions?: CanvasPresets.TrackerOptions,
  networkConfig?: AINetworkSearchConfig
): CanvasPresets.TextStream | undefined {
  const action = AIProvider.actions[id];
  if (!action || typeof action !== 'function') return;

  let stream: CanvasPresets.TextStream | undefined;
  return {
...
function actionToGenerateAnswer<T extends keyof CanvasPresets.AIActions>(
  host: EditorHost,
  id: T,
  variants?: Omit<
    Parameters<CanvasPresets.AIActions[T]>[0],
 ...
 */
function updateAIPanelConfig<T extends keyof CanvasPresets.AIActions>(
...
export function actionToHandler<T extends keyof CanvasPresets.AIActions>(
  id: T,
  generatingIcon: TemplateResult<1>,
  variants?: Omit<
    Parameters<CanvasPresets.AIActions[T]>[0],
  ...
  return (host: EditorHost) => {
    ...
    if (
      blocks.length === 1 &&
      block.model.flavour === 'nexio:image' &&
      id === 'createImage'
    ) {
