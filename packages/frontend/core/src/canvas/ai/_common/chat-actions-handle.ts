...
const ADD_TO_EDGELESS_AS_NOTE = {
  ...
  handler: async (host: EditorHost, content: string): Promise<boolean> => {
    ...
    const id = store.addBlock('nexio:note', props, store.root?.id);
...
export const SAVE_AS_DOC = {
  ...
  handler: (host: EditorHost, content: string) => {
    ...
    newDoc.load();
    const rootId = newDoc.addBlock('nexio:page');
    newDoc.addBlock('nexio:surface', {}, rootId);
    const noteId = newDoc.addBlock('nexio:note', {}, rootId);
