...
interface GroupedSessions {
  today: CanvasPresets.AIRecentSession[];
  last7Days: CanvasPresets.AIRecentSession[];
  last30Days: CanvasPresets.AIRecentSession[];
  older: CanvasPresets.AIRecentSession[];
}
...
export class AISessionHistory extends WithDisposable(ShadowlessElement) {
  ...
  accessor onSessionDelete!: (
    session: CanvasPresets.AIRecentSession
  ) => void;
  ...
  @state()
  private accessor sessions: CanvasPresets.AIRecentSession[] | undefined;
  ...
  private groupSessionsByTime(
    sessions: CanvasPresets.AIRecentSession[]
  ): GroupedSessions {
    const now = new Date();
        ...
        ${sessions.map(session => {
          return html`
            ...
            >
              ...
              >
                ${DeleteIcon()}
                <nexio-tooltip>Delete</nexio-tooltip>
              </div>
            </div>
          `;
        })}
  ...
  private renderSessionDoc(docId: string, sessionId: string) {
    ...
    >
      ...
      <nexio-tooltip>Open this doc</nexio-tooltip>
    </div>`;
  }
