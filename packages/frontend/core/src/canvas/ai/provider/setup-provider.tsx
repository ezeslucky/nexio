import { toggleGeneralAIOnboarding } from '@nexio/core/components/nexio/ai-onboarding/apis';
...
export function setupAIProvider(
  ...
  async function createSession({
    ...
    pinned,
    reuseLatestChat,
  }: CanvasPresets.AICreateSessionOptions) {
...
Could you make a new website based on these notes and send back just the html file?`;
  ...
  AIProvider.provide('context', {
    ...
    pollContextDocsAndFiles: async (
      ...
      contextId: string,
      onPoll: (
        result: CanvasPresets.AIDocsAndFilesContext | undefined
      ) => void,
  ...
  AIProvider.provide('histories', {
    actions: async (
      workspaceId: string,
      docId: string
    ): Promise<CanvasPresets.AIHistory[]> => {
    ...
    ids: async (
      ...
      >['variables']['options']
    ): Promise<CanvasPresets.AIHistoryIds[]> => {
      // @ts-expect-error - 'action' is missing in server impl
