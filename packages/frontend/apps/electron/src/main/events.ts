...
import { NEXIO_EVENT_CHANNEL_NAME } from '../shared/type';
    ...
    for (const [key, eventRegister] of Object.entries(namespaceEvents)) {
      const unsubscribe = eventRegister((...args: any[]) => {
        ...
        getActiveWindows().forEach(win => {
          ...
          win.webContents?.send(NEXIO_EVENT_CHANNEL_NAME, chan, ...args);
          win.contentView.children.forEach(child => {
            if (
              ...
              child.webContents?.send(NEXIO_EVENT_CHANNEL_NAME, chan, ...args);
            }
          });
        });
      });
