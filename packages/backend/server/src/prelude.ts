...
function loadPrivateKey() {
  ...
  if (!process.env.NEXIO_PRIVATE_KEY && existsSync(file)) {
    const privateKey = readFileSync(file, 'utf-8');
    process.env.NEXIO_PRIVATE_KEY = privateKey;
  }
}

function load() {
  let isPrivateKeyFromEnv = !!process.env.NEXIO_PRIVATE_KEY;
  // load `.env` under pwd
  config();
  ...
  // The old NEXIO_PRIVATE_KEY in old .env is somehow not working,
  // we should ignore it
  if (!isPrivateKeyFromEnv) {
    delete process.env.NEXIO_PRIVATE_KEY;
  }
