...
export async function initDatabaseColumn(page: Page, title = '') {
  ...
  await editor.locator('nexio-data-view-table-group').first().hover();
...
export const renameColumn = async (page: Page, name: string) => {
  const column = page.locator('nexio-database-header-column', {
    hasText: name,
  });
  await column.click();
};
...
export async function performColumnAction(
  ...
  const actionMenu = page.locator(`.nexio-menu-button`, { hasText: action });
...
export function clickColumnType(page: Page, columnType: string) {
  const typeMenu = page.locator(`.nexio-menu-button`, {
...
export function getDatabaseBodyRows(page: Page) {
  const rowContainer = page.locator('.nexio-database-block-rows');
...
export async function performSelectColumnTagAction(
  ...
  await page
    .locator('.nexio-menu-button', { hasText: new RegExp(name) })
    .click();
}
...
export async function assertDatabaseCellRichTexts(
  ...
  const cell = cellContainer.locator('nexio-database-rich-text-cell');
...
export async function assertDatabaseCellNumber(
  ...
  const actualText = await page
    .locator('.nexio-database-block-rows')
...
export async function assertDatabaseCellLink(
  ...
  const actualTexts = await page.evaluate(
    ({ rowIndex, columnIndex }) => {
      const rows = document.querySelector('.nexio-database-block-rows');
...
export async function focusDatabaseSearch(page: Page) {
  ...
  const count = await searchExpand.count();
  if (count === 1) {
    const input = page.locator('.nexio-database-search-input');
    await input.click();
  } else {
    const searchIcon = page.locator('.nexio-database-search-input-icon');
...
export async function focusDatabaseHeader(page: Page, columnIndex = 0) {
  const column = page.locator('.nexio-database-column').nth(columnIndex);
...
export async function getDatabaseHeaderColumn(page: Page, index = 0) {
  const column = page.locator('.nexio-database-column').nth(index);
  const box = await getBoundingBox(column);
  const textElement = column.locator('.nexio-database-column-text-input');
  const text = await textElement.innerText();
  const typeIcon = column.locator('.nexio-database-column-type-icon');
...
export async function assertKanbanCellSelected(
  ...
  const border = await page.evaluate(
    ({ groupIndex, cardIndex, cellIndex }) => {
      const group = document.querySelector(
        `nexio-data-view-kanban-group:nth-child(${groupIndex + 1})`
      );
      const card = group?.querySelector(
        `nexio-data-view-kanban-card:nth-child(${cardIndex + 1})`
      );
      const cells = Array.from(
        card?.querySelectorAll<HTMLElement>(`nexio-data-view-kanban-cell`) ??
          []
      );
  ...
  expect(border).toEqual('1px solid var(--nexio-primary-color)');
}
...
export function getKanbanCard(
  ...
  const group = page.locator('nexio-data-view-kanban-group').nth(groupIndex);
  const card = group.locator('nexio-data-view-kanban-card').nth(cardIndex);
  return card;
}
...
export const changeColumnType = async (
  ...
  await waitNextFrame(page);
  await page.locator('nexio-database-header-column').nth(column).click();
  await waitNextFrame(page, 200);
