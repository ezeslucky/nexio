...
test('can sync svg between different browsers', async ({ page, browser }) => {
  ...
  // get the image src under "nexio-image img"
  const src1 = await page.locator('nexio-image img').getAttribute('src');
...

  {
    ...
    // get the image src under "nexio-image img"
    const src2 = await page2.locator('nexio-image img').getAttribute('src');
