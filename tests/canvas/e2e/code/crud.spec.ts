...
test('use debug menu can create code block', async ({ page }) => {
  ...
  await updateBlockType(page, 'nexio:code');
...
test('support ```[lang] to add code block with language', async ({ page }) => {
  test.info().annotations.push({
    type: 'issue',
    description: 'https://github.com/ezeslucky/canvas/issues/1314',
  });
...
test('use more than three backticks can not create code block', async ({
  ...
  const codeBlockLocator = page.locator('nexio-code');
...
test('change code language can work', async ({ page }, testInfo) => {
  ...
  await page.click(
    '.nexio-filterable-list > .items-container > icon-button:nth-child(1)'
  );
...
test('toggle code block line number can work', async ({ page }) => {
  ...
  const lineNumber = page.locator('nexio-code .line-number');
...
test('code block toolbar widget can appear and disappear during mousemove', async ({
  ...
  const position = await page.locator('nexio-code').boundingBox();
  if (!position) throw new Error('Failed to get nexio code position');
  await page.mouse.move(position.x, position.y);
...
test('should bracket complete works in code block', async ({ page }) => {
  test.info().annotations.push({
    type: 'issue',
    description: 'https://github.com/ezeslucky/canvas/issues/1800',
  });
  await enterPlaygroundRoom(page);
  await initEmptyCodeBlockState(page);
  await focusRichText(page);
...
test('auto scroll horizontally when typing', async ({ page }) => {
  ...
  const richTextScrollLeft1 = await page.evaluate(() => {
    const richText = document.querySelector('nexio-code rich-text');
    if (!richText) {
...
test('format text in code block', async ({ page }, testInfo) => {
  ...
  const line = page.locator('nexio-code rich-text v-line > div');
  ...
  const linkLocator = page.locator('[data-block-id="3"] nexio-link a');
