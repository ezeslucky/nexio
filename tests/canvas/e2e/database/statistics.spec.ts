...
const insertRightColumn = async (page: Page, index = 0) => {
  await waitNextFrame(page);
  await page.locator('nexio-database-header-column').nth(index).click();
  await waitNextFrame(page, 200);
  await pressKey(page, 'Escape');
  const menu = page.locator('.nexio-menu-button', {
...
test.describe('title', () => {
  test('empty count', async ({ page }) => {
    ...
    await addRow(page, 3);
    const statCell = page.locator('nexio-database-column-stats-cell').nth(0);
...
test.describe('rich-text', () => {
  test('empty count', async ({ page }) => {
    ...
    const statCell = page.locator('nexio-database-column-stats-cell').nth(1);
    await moveToCenterOf(page, statCell);
    await statCell.click();
    await menuSelect(page, ['Count', 'Count Empty']);
    const value = statCell.locator('.value');
