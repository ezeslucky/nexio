...
test('select text in the same line with dragging leftward and move outside the nexio-note', async ({
  page,
}) => {
  await enterPlaygroundRoom(page);
  await initEmptyParagraphState(page);
  ...
  const noteLeft = await page.evaluate(() => {
    const note = document.querySelector('nexio-note');
    if (!note) {
      throw new Error();
    }
...
test('select text in the same line with dragging leftward and move outside the nexio-note by forwardDelete', async ({
  page,
}) => {
  await enterPlaygroundRoom(page);
  await initEmptyParagraphState(page);
...
test('select text in the same line with dragging rightward and move outside the nexio-note', async ({
  page,
}) => {
  await enterPlaygroundRoom(page);
  await initEmptyParagraphState(page);
...
test('select text in the same line with dragging rightward and move outside the nexio-note by forwardDelete', async ({
  page,
}) => {
  await enterPlaygroundRoom(page);
  await initEmptyParagraphState(page);
...
test('should not crash when mouse over the left side of the list block prefix', async ({
  ...
  const prefixIconRect = await page.evaluate(() => {
    ...
    const prefixIcon = block.querySelector('.nexio-list-block__prefix ');
    if (!prefixIcon) {
      throw new Error();
    }
...
test('should set the last block to end the range after when leaving the nexio-note', async ({
  page,
}) => {
  await enterPlaygroundRoom(page);
  await initEmptyParagraphState(page);
...
test('should set the first block to start the range before when leaving the nexio-note-block-container', async ({
  page,
}) => {
  await enterPlaygroundRoom(page);
  await initEmptyParagraphState(page);
...
test('should select full text of the first block when leaving the nexio-note-block-container in edgeless mode', async ({
  page,
}) => {
  await enterPlaygroundRoom(page);
  ...
  const containerRect = await page.evaluate(() => {
    const container = document.querySelector('.nexio-note-block-container');
    if (!container) {
      throw new Error();
    }
...
test('should add a new line when clicking the bottom of the last non-text block', async ({
  ...
  const locator = page.locator('nexio-code');
...
test('should clear native selection before block selection', async ({
  ...
  const rects = page.locator('nexio-block-selection').locator('visible=true');
...
test('should keep native range selection when scrolling backward with the scroll wheel', async ({
  ...
  const blockHeight = await page.evaluate(() => {
    const viewport = document.querySelector('.nexio-page-viewport');
    if (!viewport) {
      throw new Error();
    }
    ...
    const container = viewport.querySelector(
      'nexio-note .nexio-block-children-container'
    );
    if (!container) {
      throw new Error();
    }
...
test('should keep native range selection when scrolling forward with the scroll wheel', async ({
  ...
  await page.evaluate(() => {
    document.querySelector('.nexio-page-viewport')?.scrollTo(0, 0);
  });
  await page.mouse.move(0, 0);
...
test('click to select divided', async ({ page }) => {
  ...
  await page.click('nexio-divider');
  const selectedBlocks = page
    .locator('nexio-block-selection')
...
test('abnormal cursor jumping', async ({ page }) => {
  ...
  await pressEnter(page);
  await page.locator('nexio-image block-zero-width .block-zero-width').click();
  await pressArrowUp(page);
  await pressTab(page);
  await pressArrowDown(page);
  await pressTab(page);
...
test('unexpected scroll when clicking padding area', async ({ page }) => {
  ...
  const database = page.locator('nexio-database');
