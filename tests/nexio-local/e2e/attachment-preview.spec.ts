...
test('attachment preview should be shown', async ({ page }) => {
  ...
  await page.locator('nexio-attachment').first().dblclick();
...
test('should preview PDF in embed view', async ({ page }) => {
  ...
  // Title alias
  {
    ...
    const inlineLink = page.locator('nexio-reference').nth(0);
    const inlineTitle = inlineLink.locator('.nexio-reference-title');
  ...
  {
    const inlineLink = page.locator('nexio-reference').nth(1);
    const inlineTitle = inlineLink.locator('.nexio-reference-title');
...
test('should sync name in pdf embed view', async ({ page }) => {
  ...
  const input = page
    .locator('.nexio-attachment-rename-input-wrapper')
    .locator('input');
...
test('should enable pointer event in pdf viewer', async ({ page }) => {
  ...
  const attachmentSelection = attachment.locator(
    '.nexio-attachment-container.focused'
  );
  ...
  await attachment
    .locator('.nexio-attachment-embed-container')
