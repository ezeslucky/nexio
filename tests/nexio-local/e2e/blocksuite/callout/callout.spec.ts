...
test('add callout block using slash menu and change emoji', async ({
  ...
  const callout = page.locator('nexio-callout');
  const emoji = page.locator('nexio-callout').getByTestId('callout-emoji');
  await expect(callout).toBeVisible();
  await expect(emoji).toContainText('💡');

  const paragraph = page.locator('nexio-callout nexio-paragraph');
  await expect(paragraph).toHaveCount(2);

  const vLine = page.locator('nexio-callout v-line');
