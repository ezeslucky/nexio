...
test.describe('edgeless page block', () => {
  ...
  test('should shrink note block when clicking on the toggle button', async ({
    ...
    const noteBox = await page.locator('nexio-edgeless-note').boundingBox();
    ...
    const newNoteBox = await page.locator('nexio-edgeless-note').boundingBox();
  ...
  test('info button should hidden in peek view', async ({ page }) => {
    ...
    const reference = page.locator('nexio-reference');
  ...
  test('page title should be editable', async ({ page }) => {
    ...
    const paragraphs = note.locator('nexio-paragraph v-line');
...
test.describe('edgeless note element toolbar', () => {
  ...
  test('the toolbar of note block should contains auto-height button and display in page button', async ({
    ...
    await createEdgelessNoteBlock(page, [100, 100]);
    await page.waitForSelector('.nexio-paragraph-placeholder.visible');
  ...
  test('display in page button', async ({ page }) => {
    ...
    const notes = editorContainer.locator('nexio-note');
    ...
    const toc = page.locator('nexio-outline-panel');
    await toc.waitFor({ state: 'visible' });
    const highlightNoteCards = toc.locator(
      'nexio-outline-note-card > [data-status="selected"]'
    );
  ...
  test('note edgeless styles', async ({ page }) => {
    ...
    await noteStylePanel.getByTestId('nexio-note-shadow-film').click();
    ...
    expect(await getNoteEdgelessProps(page, noteId)).toEqual({
      style: {
        ...
        borderSize: 4,
        borderStyle: 'none',
        shadowType: '--nexio-note-shadow-film',
      },
    });

    const borderStylePanel = noteStylePanel.getByTestId(
      'nexio-note-border-style-panel'
    );
    await borderStylePanel.locator('.mode-solid').click();
    await borderStylePanel.locator('nexio-slider').getByLabel('8').click();
    ...
    const cornerPanel = noteStylePanel.getByTestId(
      'nexio-note-corner-radius-panel'
    );
    await cornerPanel.locator('nexio-slider').getByLabel('24').click();
...
test('should convert note block to linked doc when clicking turn into linked doc button', async ({
  ...
  const syncedDoc = page.locator('nexio-embed-edgeless-synced-doc-block');
  await expect(syncedDoc).toBeVisible();

  const noteInSyncedDoc = syncedDoc.locator('nexio-note');
  await expect(noteInSyncedDoc).toBeVisible();

  const paragraphs = noteInSyncedDoc.locator('nexio-paragraph');
