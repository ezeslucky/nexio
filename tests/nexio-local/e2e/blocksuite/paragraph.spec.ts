...
test('heading icon should be updated after change heading level', async ({
  ...
  const paragraph = page.locator('nexio-note nexio-paragraph').nth(0);
...
test('basic heading collapsed', async ({ page }) => {
  ...
  const a = page.locator('nexio-note nexio-paragraph').nth(0);
  const b = page.locator('nexio-note nexio-paragraph').nth(1);
  const c = page.locator('nexio-note nexio-paragraph').nth(2);
  const d = page.locator('nexio-note nexio-paragraph').nth(3);
  const e = page.locator('nexio-note nexio-paragraph').nth(4);
  const f = page.locator('nexio-note nexio-paragraph').nth(5);
  const g = page.locator('nexio-note nexio-paragraph').nth(6);
  const h = page.locator('nexio-note nexio-paragraph').nth(7);
  const i = page.locator('nexio-note nexio-paragraph').nth(8);
  const j = page.locator('nexio-note nexio-paragraph').nth(9);
...
test('also move children when dedent collapsed heading', async ({ page }) => {
  ...
  const paragraph = page.locator('nexio-note nexio-paragraph');
  const subParagraph = paragraph.nth(0).locator('nexio-paragraph');
...
test('also move collapsed siblings when indent collapsed heading', async ({
  ...
  const paragraph = page.locator('nexio-note nexio-paragraph');
  expect(await paragraph.nth(0).locator('nexio-paragraph').count()).toBe(0);
  await pressTab(page);
  expect(await paragraph.nth(0).locator('nexio-paragraph').count()).toBe(1);
  expect(
    await paragraph
      .nth(0)
      .locator('nexio-paragraph')
      .nth(0)
      .evaluate(
        (block: ParagraphBlockComponent) =>
          block.model.props.type === 'h1' &&
  ...
  await pressTab(page);
  expect(await paragraph.nth(2).isVisible()).toBeFalsy();
  expect(await paragraph.nth(0).locator('nexio-paragraph').count()).toBe(2);
  expect(
    await paragraph
      .nth(0)
      .locator('nexio-paragraph')
...
test('unfold collapsed heading when its other blocks indented to be its sibling', async ({
  ...
  expect(await paragraph.nth(0).locator('nexio-paragraph').count()).toBe(2);
  await pressTab(page);
  expect(await paragraph.nth(0).locator('nexio-paragraph').count()).toBe(3);
  expect(await paragraph.nth(2).isVisible()).toBeTruthy();
});
