...
test.describe('split list', () => {
  test('split at the start of the list', async ({ page }) => {
    ...
    const listLocator = page.locator('nexio-list');
    ...
    await expect(
      listLocator.nth(0).locator('.nexio-list-block__numbered')
    ).toHaveText(['1.', 'a.']);
    await expect(listLocator.nth(2).locator('rich-text')).toHaveText('ccc');
    await expect(
      listLocator.nth(2).locator('.nexio-list-block__numbered')
    ).toHaveText('2.');
    await expect(listLocator.nth(3).locator('rich-text')).toHaveText('ddd');
    await expect(
      listLocator.nth(3).locator('.nexio-list-block__numbered')
    ).toHaveText('3.');
    ...
    await expect(
      listLocator.nth(1).locator('.nexio-list-block__numbered')
    ).toHaveText(['2.', 'a.']);
    ...
    await expect(
      listLocator.nth(4).locator('.nexio-list-block__numbered')
    ).toHaveText('4.');
  });
  ...
  test('tab in list should not propagate out of editor', async ({ page }) => {
    ...
    const list = page.locator('nexio-list');
    const card = page.locator('nexio-embed-linked-doc-block');
