...
const TEST_NEXIO_URL = 'https://nexio.pro/';

const EMBED_IFRAME_BLOCK = 'nexio-embed-iframe-block';
const EMBED_EDGELESS_IFRAME_BLOCK = 'nexio-embed-edgeless-iframe-block';
...
test.beforeEach(async ({ page }) => {
  ...
  await page.locator('nexio-paragraph v-line div').click();
});
...
test.describe('embed iframe block', () => {
  ...
  test('add bookmark when the url cannot be embedded', async ({ page }) => {
    ...
    await input.fill(TEST_NEXIO_URL);
    await input.press('Enter');
    ...
    const bookmarkBlock = page.locator('nexio-bookmark');
  ...
  test.describe('conversions', () => {
    ...
    test('embed iframe block to link text', async ({ page }) => {
      ...
      const nexioLink = page.locator('nexio-link');
      await expect(nexioLink).toBeVisible();

      // hover nexio link
      await nexioLink.hover();
      await page.waitForTimeout(100);
      ...
      await expect(nexioLink).toHaveCount(0);
    });
  ...
  test.describe('edgeless mode', () => {
    ...
    test('convert between embed view and card view', async ({ page }) => {
      ...
      // expect the bookmark block count is 1
      const bookmarkEdgelessBlock = page.locator('nexio-edgeless-bookmark');
