...
test('can show database backlink info', async ({ page }) => {
  ...
  await expect(page.locator('nexio-database-title')).toContainText(
    databaseTitle
  );

  await expect(
    page.locator(`nexio-database-title:has-text("${databaseTitle}")`)
  ).toBeVisible();
  ...
  await page.locator('.nexio-reference-title:has-text("linked page")').click();
  ...
  await expect(
    linkedDatabaseSection.locator(
      `.nexio-reference-title:has-text("${pageTitle}")`
    )
  ).toBeVisible();
