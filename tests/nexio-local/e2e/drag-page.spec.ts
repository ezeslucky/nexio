...
test.skip('drag a page link in editor to favourites', async ({ page }) => {
  ...
  const pageReference = page.locator('a').filter({
    has: page.locator(
      '.nexio-reference-title:has-text("hi from another page")'
    ),
  });
  ...
  await dragToFavourites(
    page,
    page.locator('.nexio-reference-title:has-text("hi from another page")'),
    pageId
  );
});
...
test('drag a page card block to another page', async ({ page }) => {
  ...
  const box = await page.locator('nexio-embed-linked-doc-block').boundingBox();
  ...
  await dragToFavourites(
    page,
    page.locator('.nexio-drag-handle-container'),
    pageId
  );
});
...
test('drag a favourite page into note on page mode', async ({ page }) => {
  ...
  await dragTo(
    page,
    item,
    page.locator('.nexio-paragraph-block-container').first()
  );

  await expect(page.locator('nexio-embed-linked-doc-block')).toContainText(
    'hi from page'
  );
});
...
test('drag a favourite page into canvas on edgeless mode', async ({ page }) => {
  ...
  const cardDoc = page.locator('nexio-embed-edgeless-linked-doc-block');
...
test('drag a favourite page into note on edgeless mode', async ({ page }) => {
  ...
  const edgelessNote = page.locator('nexio-edgeless-note');
  await dragTo(page, item, edgelessNote);

  const embedDocInNote = edgelessNote.locator('nexio-embed-synced-doc-block');
  const cardDocInNote = edgelessNote.locator('nexio-embed-linked-doc-block');
