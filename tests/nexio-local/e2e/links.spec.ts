...
test('not allowed to switch to embed view when linking to the same document', async ({
  ...
  // Inline
  const inlineLink = page.locator('nexio-reference');
  ...
  // Card
  const cardLink = page.locator('nexio-embed-linked-doc-block');
...
test('not allowed to switch to embed view when linking to block', async ({
  ...
  const otherCardLink = page.locator('nexio-embed-linked-doc-block').nth(1);
...
test('allow switching to embed view when linking to the other document without mode', async ({
  ...
  // Embed
  const embedLink = page.locator('nexio-embed-synced-doc-block');
  ...
  await switchViewBtn.click();
  await expect(
    page.locator('.nexio-embed-synced-doc-container.page')
  ).toBeVisible();
...
test('allow switching to embed view when linking to the other document with mode', async ({
  ...
  await switchViewBtn.click();
  await expect(
    page.locator('.nexio-embed-synced-doc-container.edgeless')
  ).toBeVisible();
...
test('@ popover should show today menu item', async ({ page }) => {
  ...
  // a nexio-reference should be created with name date
  await expect(
    page.locator('nexio-reference:has-text("' + date + '")')
  ).toBeVisible();
});
...
test('@ popover with input=tmr', async ({ page }) => {
  ...
  // a nexio-reference should be created with name date
  await expect(
    page.locator('nexio-reference:has-text("' + textContent + '")')
  ).toBeVisible();
});
...
test('the viewport should be fit when the linked document is with edgeless mode', async ({
  ...
  const note = page.locator('nexio-edgeless-note');
  ...
  await page.waitForSelector('nexio-edgeless-text');
  ...
  // Inline
  await page.locator('nexio-reference').hover();
  await page.getByLabel('Switch view').click();
  await page.getByTestId('link-to-embed').click();

  const viewport = await page
    .locator('nexio-embed-synced-doc-block')
    .boundingBox();
  expect(viewport).not.toBeNull();
  if (!viewport) return;

  const edgelessText = await page
    .locator('nexio-embed-synced-doc-block nexio-edgeless-text')
    .boundingBox();
...
test('should show edgeless content when switching card view of linked mode doc in edgeless', async ({
  ...
  await page.locator('nexio-edgeless-text').waitFor({ state: 'visible' });
  await page.keyboard.type('Edgeless Text');
  ...
  // Inline
  const embed = page.locator('nexio-embed-edgeless-linked-doc-block');
  await expect(embed).toBeVisible();
  await page.getByLabel('Switch view').click();
  await page.getByTestId('link-to-embed').click();

  const viewport = await page
    .locator('nexio-embed-edgeless-synced-doc-block')
    .boundingBox();
  expect(viewport).not.toBeNull();
  if (!viewport) return;

  const edgelessText = await page
    .locator('nexio-embed-edgeless-synced-doc-block nexio-edgeless-text')
    .boundingBox();
  expect(edgelessText).not.toBeNull();
  if (!edgelessText) return;
...
test.describe('Customize linked doc title and description', () => {
  ...
  test('should set a custom title for inline link', async ({ page }) => {
    ...
    const link = page.locator('nexio-reference');
    const title = link.locator('.nexio-reference-title');
  ...
  test('should set a custom title and description for card link', async ({
    ...
    const cardTitle = cardLink.locator(
      '.nexio-embed-linked-doc-content-title-text'
    );
    const cardDescription = cardLink.locator(
      '.nexio-embed-linked-doc-content-note.alias'
    );
    ...
    // Switches to inline view
    {
      ...
      const title = inlineLink.locator('.nexio-reference-title');
  ...
  //   await expect(a.locator('.nexio-reference-title')).toHaveText('hello');
  // });
  ...
  //     await expect(a.locator('svg')).toBeHidden();
  //     await expect(a.locator('.nexio-reference-title')).toContainText(year);
  //   });
});
...
test('should show full email address', async ({ page }) => {
  ...
  await writeTextToClipboard(page, 'dev@nexio.pro');

  const inlineLink = page.locator('nexio-link');

  await expect(inlineLink).toHaveText('dev@nexio.pro');
  ...
  await expect(toolbar.locator('nexio-link-preview')).toHaveText(
    'dev@nexio.pro'
  );
});
...
test('should not show view toggle button when protocol of link is not http(s)', async ({
  ...
  await expect(toolbar.locator('nexio-link-preview')).toHaveText('nexio.pro');
});

test('should reach target block when clicking nexio-link multiple times', async ({
  page,
}) => {
  await page.keyboard.press('Enter');
  ...
  const paragraph = page.locator('nexio-paragraph').nth(0);
  ...
  await page.waitForSelector('.nexio-link-popover');

  await pasteByKeyboard(page);

  await page.locator('.nexio-confirm-button').click();

  const scrollAnchoringWidget = page.locator('nexio-scroll-anchoring-widget');
...
test('should display date as the original title of journal', async ({
  ...
  const linkedDocTitle = toolbar.locator('nexio-linked-doc-title .label');
...
test('should add HTTP protocol into link automatically', async ({ page }) => {
  ...
  const linkPreview = toolbar.locator('nexio-link-preview');
  ...
  const cardLink = page.locator('nexio-bookmark');
...
test('should open link in new tab when middle clicking on link', async ({
  ...
  // external link
  {
    ...
    const input = page.locator('.nexio-link-popover-input');
    ...
    await page.locator('nexio-link').click({ button: 'middle' });
  ...
  // reference doc
  {
    ...
    await page.locator('nexio-reference').click({ button: 'middle' });
