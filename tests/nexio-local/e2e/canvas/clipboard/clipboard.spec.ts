...
const codeBlockLocator = 'nexio-note nexio-code';
...
test.describe('surface-ref block', () => {
  async function setupSurfaceRefBlock(page: Page) {
    ...
    const frameTitle = page.locator('nexio-frame-title');
  ...
  test('paste surface-ref block to another doc as embed-linked-doc block', async ({
    ...
    const surfaceRefBlock = page.locator('nexio-surface-ref');
    await surfaceRefBlock.click();
    await page.waitForSelector('nexio-surface-ref .focused');
    await copyByKeyboard(page);
    ...
    const embedLinkedDocBlock = page.locator('nexio-embed-linked-doc-block');
...
test('should copy single image from edgeless and paste to page', async ({
  ...
  const image = page.locator('nexio-edgeless-image').first();
  await image.click();
  ...
  await expect(page.locator('nexio-page-image')).toBeVisible();
});
