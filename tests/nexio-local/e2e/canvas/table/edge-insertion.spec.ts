...
import { createTable, getCellText } from '@nexio-test/kit/bs/table';
...
test.describe('Table edge case insertion operations', () => {
  test('should correctly insert at edges', async ({ page }) => {
    ...
    const table = page.locator('nexio-table');
    ...
    const cells = table.locator('nexio-table-cell');
    ...
    const menu = page.locator('nexio-menu');
    await expect(menu).toBeVisible();
    await menu.getByText('Insert Left').click();
    await waitNextFrame(page);

    // Verify: Now we should have 6 cells (3x2 table)
    const cellsAfterLeftEdgeInsert = table.locator('nexio-table-cell');
    ...
    const cellsAfterAboveEdgeInsert = table.locator('nexio-table-cell');
    ...
    const cellsAfterRightEdgeInsert = table.locator('nexio-table-cell');
    ...
    const cellsAfterBelowEdgeInsert = table.locator('nexio-table-cell');
