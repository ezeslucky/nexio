...
test.describe('Formatting', () => {
  test('should change text color', async ({ page }) => {
    ...
    const toolbar = locateToolbar(page);
    const highlightButton = toolbar.locator('nexio-highlight-duotone-icon');
    ...
    const fgColor = await fgGreenButton
      .locator('nexio-text-duotone-icon')
      .evaluate(e => window.getComputedStyle(e).getPropertyValue('--color'));

    const paragraph = page.locator('nexio-paragraph');
    const textSpan = paragraph
      .locator('nexio-text:has-text("rld")')
      .locator('span')
      .first();
...
test('should not show inner toolbar of surface-ref in note under edgeless', async ({
  ...
  const surfaceRef = page.locator('nexio-surface-ref');
  ...
  const edgelessSurfaceRef = note.locator('nexio-edgeless-surface-ref');
  await edgelessSurfaceRef.hover();

  await expect(toolbar).toBeHidden();

  const dragHandler = page.locator('.nexio-drag-handle-grabber');
  await dragHandler.hover();
  await dragHandler.click();

  await expect(
    edgelessSurfaceRef.locator('.nexio-edgeless-surface-ref-container')
  ).toHaveClass(/focused$/);
...
test('should focus on input of popover on toolbar', async ({ page }) => {
  ...
  const note = page.locator('nexio-edgeless-note').first();
...
test.describe('Toolbar More Actions', () => {
  test('should duplicate block', async ({ page }) => {
    ...
    const images = page.locator('nexio-page-image');
