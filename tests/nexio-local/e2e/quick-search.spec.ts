...
test('assert the recent browse pages are on the recent list', async ({
  ...
  await waitForEditorLoad(page);
  {
    ...
    await expect(title).toHaveText('nexio is the best', { timeout: 500 });
  }
  ...
  await openQuickSearchByShortcut(page);
  {
    ...
    await expect(quickSearchItems.nth(0)).toHaveText('nexio is the best');
  }
});
...
test('can use cmdk to search page content and scroll to it, then the block will be selected', async ({
  ...
  const textBlock = page
    .locator('[data-nexio-editor-container]')
    .getByText('123456');
  ...
  const isVisitable = await checkElementIsInView(
    page,
    page.locator('[data-nexio-editor-container]').getByText('123456')
  );
  expect(isVisitable).toBe(true);
  const selectionElement = page.locator(
    'nexio-scroll-anchoring-widget div.highlight'
  );
...
test('disable quick search when the link-popup is visitable', async ({
  ...
  await withCtrlOrMeta(page, () => page.keyboard.press('k'));
  const linkPopup = page.locator('.nexio-link-popover');
...
test('can use @ to open quick search to search for doc and insert into canvas', async ({
  ...
  await page.locator('nexio-edgeless-root').press('@');
  ...
  await page.keyboard.press('Enter');
  await expect(
    page.locator('nexio-embed-edgeless-synced-doc-block')
  ).toBeVisible();
  await expect(
...
test('can paste a doc link to create link reference', async ({ page }) => {
  ...
  await expect(
    page.locator('nexio-reference:has-text("Getting Started")')
  ).toBeVisible();

  // can ctrl-z to revert to normal link
  await page.keyboard.press('ControlOrMeta+z');

  await expect(page.locator(`nexio-link:has-text("${url}")`)).toBeVisible();
});
...
test('can use slash menu to insert an external link', async ({ page }) => {
  ...
  await expect(page.locator('nexio-bookmark')).toBeVisible();
  await expect(page.locator('.nexio-bookmark-content-url')).toContainText(
    link
  );
});
