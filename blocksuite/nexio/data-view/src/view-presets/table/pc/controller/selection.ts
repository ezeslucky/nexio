...
export class TableSelectionController implements ReactiveController {
  ...
  focusToCell(position: 'left' | 'right' | 'up' | 'down') {
    ...
    const rows = Array.from(
      row
        ?.closest('.nexio-database-table-container')
    ...
    const container =
      groupKey != null
        ? this.tableContainer?.querySelector(
            `nexio-data-view-table-group[data-group-key="${groupKey}"]`
          )
        : this.tableContainer;
  ...
  startDrag(
    ...
    const groupKey = cell.closest<TableGroup>('nexio-data-view-table-group')
      ?.group?.key;
...
export class SelectionElement extends WithDisposable(ShadowlessElement) {
  ...
  updateFocusSelectionStyle(
    ...
    div.style.borderColor = 'var(--nexio-primary-color)';
