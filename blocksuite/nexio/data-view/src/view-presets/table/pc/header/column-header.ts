import { getScrollContainer } from '@blocksuite/nexio-shared/utils';
...
export class DatabaseColumnHeader extends SignalWatcher(
  ...
  editLastColumnTitle = () => {
    const columns = this.querySelectorAll('nexio-database-header-column');
  ...
  override connectedCallback() {
    ...
    const scrollContainer = getScrollContainer(
      this.closest('nexio-data-view-renderer')!
    );
    const group = this.closest('nexio-data-view-table-group');
    if (group) {
  ...
  override render() {
    return html`
      ...
      <div class="nexio-database-column-header database-row">
        ${this.readonly
          ? nothing
          ...
          (column, index) => {
            ...
            return html`
              <nexio-database-header-column
                ...
                class="nexio-database-column database-cell"
                .column="${column}"
...
declare global {
  interface HTMLElementTagNameMap {
    'nexio-database-column-header': DatabaseColumnHeader;
  }
}
