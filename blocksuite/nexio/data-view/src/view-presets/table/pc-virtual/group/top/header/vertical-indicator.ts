...
export const getTableGroupRect = (ele: HTMLElement) => {
  const group = ele.closest('nexio-data-view-virtual-table-group');
  if (!group) {
    return;
  }
  ...
  const top =
    group
      .querySelector('.nexio-database-column-header')
      ?.getBoundingClientRect().top ?? groupRect.top;
  const bottom =
    group.querySelector('.nexio-database-block-rows')?.getBoundingClientRect()
