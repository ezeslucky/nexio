import { getRangeByPositions } from '@blocksuite/nexio-shared/utils';
...
export class TableSelectionController implements ReactiveController {
  ...
  private resolveDragStartTarget(
    ...
    if (fillValues) {
      ...
      cell = target.closest('nexio-database-virtual-cell-container');
    }
  ...
  getCellElement(cell: TableGridCell): DatabaseCellContainer | undefined {
    return (
      cell.element.querySelector('nexio-database-virtual-cell-container') ??
      undefined
    );
  }
...
export class SelectionElement extends SignalWatcher(
  ...
  static override styles = css`
    ...
    .database-selection {
      ...
      background-color: var(--nexio-primary-color-04);
      pointer-events: none;
      display: none;
    }
