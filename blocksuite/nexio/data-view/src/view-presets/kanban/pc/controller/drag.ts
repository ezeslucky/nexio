import type { InsertToPosition } from '@blocksuite/nexio-shared/utils';
...
export class KanbanDragController implements ReactiveController {
  dragStart = (ele: KanbanCard, evt: PointerEvent) => {
    ...
    const currentGroup = ele.closest('nexio-data-view-kanban-group');
      ...
      this.host.disposables.add(
        this.logic.handleEvent('dragStart', context => {
          ...
          if (target instanceof Element) {
            const cell = target.closest('nexio-data-view-kanban-cell');
            if (cell?.isEditing$.value) {
              return;
            }
            cell?.selectCurrentCell(false);
            const card = target.closest('nexio-data-view-kanban-card');
            if (card) {
...
const createDragPreview = (card: KanbanCard, x: number, y: number) => {
  ...
  kanbanCard.style.backgroundColor = 'var(--nexio-background-primary-color)';
...
const createDropPreview = () => {
  ...
  div.style.backgroundColor = 'var(--nexio-primary-color)';
