...
const styles = css`
  .nexio-database-search-container {
    position: relative;
    display: flex;
    align-items: center;
    gap: 8px;
  ...
  .nexio-database-search-input-icon {
    position: absolute;
    left: 0;
    font-size: 20px;
    display: flex;
  ...
  .nexio-database-search-input-icon:hover {
    background: var(--nexio-hover-color);
  }

  .search-container-expand .nexio-database-search-input-icon {
    left: 4px;
    pointer-events: none;
  }

  .nexio-database-search-input {
    flex: 1;
    width: 100%;
  ...
  .nexio-database-search-input::placeholder {
...
> {
  ...
  override render() {
    const searchToolClassMap = classMap({
      'nexio-database-search-container': true,
      'search-container-expand': this.showSearch,
      active: this.showSearch,
    });
    return html`
      <label class="${searchToolClassMap}" @click="${this._clickSearch}">
        <div class="nexio-database-search-input-icon">${SearchIcon()}</div>
        <input
          placeholder="Search..."
          class="nexio-database-search-input"
  ...
  @query('.nexio-database-search-input')
