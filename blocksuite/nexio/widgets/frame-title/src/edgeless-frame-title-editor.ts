import { FrameBlockModel } from '@blocksuite/nexio-model';
...
import {
  ...
  type NexioFrameTitleWidget,
} from './nexio-frame-title-widget';
import type { NexioFrameTitle } from './frame-title';
...
export class EdgelessFrameTitleEditor extends WithDisposable(
  ...
  override render() {
    ...
    const frameTitle =
      frameTitleWidget.querySelector<NexioFrameTitle>('nexio-frame-title');

    const colors = frameTitle?.colors ?? {
      background: cssVarV2('edgeless/frame/background/white'),
      text: 'var(--nexio-text-primary-color)',
    };
