...
} from '@blocksuite/nexio-block-embed';
import { ParagraphBlockComponent } from '@blocksuite/nexio-block-paragraph';
import { DropIndicator } from '@blocksuite/nexio-components/drop-indicator';
import {
...
import { surfaceRefToEmbed } from '@blocksuite/nexio-shared/adapters';
import {
...
export class DragEventWatcher {
    ...
    if (edge === 'right' && matchModels(dropModel, [ListBlockModel])) {
      ...
      if (
        snapshot.content.every(block =>
          schema.safeValidate(block.flavour, 'nexio:list')
        )
      ) {
      ...
      const placement =
        isDropOnNoteBlock &&
        schema.safeValidate(snapshot.content[0].flavour, 'nexio:note')
          ? 'in'
          : edge === 'top'
            ? 'before'
            : 'after';
  ...
  private readonly _onPageDrop = (
    ...
    if (
      ...
      snapshot.content.every(block => block.flavour === 'nexio:note')
    ) {
    ...
    if (
      ...
      if (dragPayload.from?.docId === this.widget.store.id) {
        ...
        const walk = (block: BlockSnapshot) => {
          if (block.flavour === 'nexio:surface') {
            Object.values(
  ...
  private readonly _rewriteSnapshotXYWH = (
    ...
    const rewrite = (block: BlockSnapshot) => {
      if (block.flavour === 'nexio:surface') {
        ...
        if (block.flavour === 'nexio:embed-iframe') {
          ...
          block.flavour === 'nexio:attachment' ||
          block.flavour === 'nexio:bookmark' ||
          block.flavour.startsWith('nexio:embed-')
        ) {
  ...
  private readonly _dropAsGfxBlock = (
    ...
    const groupByParent = groupBy(snapshot.content, block =>
      schema.safeValidate(block.flavour, 'nexio:surface')
        ? 'nexio:surface'
        : schema.safeValidate(block.flavour, 'nexio:page')
          ? 'nexio:page'
    ...
    if (!groupByParent.empty?.length) {
      ...
      if (groupByParent['nexio:surface']) {
        const content = groupByParent['nexio:surface'];
      ...
      if (groupByParent['nexio:page']) {
        const content = groupByParent['nexio:page'];
        const pageSnapshot = {
          ...snapshot,
          content,
        };
      ...
      const content = snapshot.content.filter(block =>
        schema.safeValidate(block.flavour, 'nexio:note')
      );
  ...
  private async _dropToModel(
    ...
    try {
      ...
      if (snapshot.content.length === 1) {
        const [first] = snapshot.content;
        if (first.flavour === 'nexio:embed-linked-doc') {
  ...
  private _makeDropTarget(view: BlockComponent) {
    ...
    if (
      // nexio:surface block can't be drop target in any modes
