import { DatabaseSelection } from '@blocksuite/nexio-block-database';
import { EdgelessLegacySlotIdentifier } from '@blocksuite/nexio-block-surface';
import { TableSelection } from '@blocksuite/nexio-block-table';
import {
  darkToolbarStyles,
...
export const NEXIO_TOOLBAR_WIDGET = 'nexio-toolbar-widget';

export class NexioToolbarWidget extends WidgetComponent {
  ...
  updateWithSurface(
    ...
    if (activated && surface) {
      ...
      if (hasLocked) {
        flavour = 'nexio:surface:locked';
      } else {
        if (paired.length === 1) {
          flavour = paired[0][0];
          if (flavour === 'nexio:surface:shape' && paired[0][1].length === 1) {
      ...
      if (!sideOptions) {
        ...
        if (flavours.has('nexio:surface:frame')) {
          sideOptions = sideMap.get('nexio:surface:frame') ?? null;
        } else if (flavours.has('nexio:surface:group')) {
          sideOptions = sideMap.get('nexio:surface:group') ?? null;
        }
      }
    }
    ...
    disposables.add(
      std.selection.find$(TextSelection).subscribe(result => {
        ...
        batch(() => {
          ...
          flavour$.value = 'nexio:note';
          placement$.value = toolbarRegistry.getModulePlacement('nexio:note');
      ...
      std.selection.filter$(BlockSelection).subscribe(selections => {
        ...
        batch(() => {
          ...
          placement$.value = toolbarRegistry.getModulePlacement(
            flavour,
            flavour === 'nexio:note' ? 'top' : 'top-start'
          );
