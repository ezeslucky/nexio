...
import { SmallDocIcon } from '@blocksuite/nexio-components/icons';
import {
  FrameBlockModel,
  NoteBlockModel,
  NoteDisplayMode,
...
import { matchModels, stopPropagation } from '@blocksuite/nexio-shared/utils';
...
export const NEXIO_EDGELESS_AUTO_CONNECT_WIDGET =
  'nexio-edgeless-auto-connect-widget';
...
export class EdgelessAutoConnectWidget extends WidgetComponent<RootBlockModel> {
    ...
    return html`${repeat(
      ...
      note => {
        ...
        const style = styleMap({
          ...
          borderRadius: '50%',
          backgroundColor: 'var(--nexio-text-secondary-color)',
        ...
        return html`<div style=${style} class="edgeless-only-index-label">
          ...
          <nexio-tooltip tip-position="bottom">
  ...
  private _initLabels() {
    ...
    const surfaceRefs = this.block.std.store
      .getBlocksByFlavour('nexio:surface-ref')
    ...
    this._disposables.add(
      this.store.slots.blockUpdated.subscribe(payload => {
        if (payload.flavour === 'nexio:surface-ref') {
          switch (payload.type) {
            case 'add':
...
export const autoConnectWidget = WidgetViewExtension(
  ...
  literal`${unsafeStatic(NEXIO_EDGELESS_AUTO_CONNECT_WIDGET)}`
);

declare global {
  interface HTMLElementTagNameMap {
    'nexio-edgeless-auto-connect-widget': EdgelessAutoConnectWidget;
  }
}
