...
import { ViewportElementProvider } from '@blocksuite/nexio-shared/services';
import {
  autoScroll,
  getScrollContainer,
...
export const NEXIO_PAGE_DRAGGING_AREA_WIDGET =
  'nexio-page-dragging-area-widget';

export class NexioPageDraggingAreaWidget extends WidgetComponent<RootBlockModel> {
  static excludeFlavours: string[] = ['nexio:note', 'nexio:surface'];
    ...
    const getAllNodeFromTree = (): BlockComponent[] => {
      ...
      this.host.view.walkThrough(node => {
        ...
        if (
          ...
          !NexioPageDraggingAreaWidget.excludeFlavours.includes(
            view.model.flavour
          )
        ) {
          blocks.push(view);
  ...
  override render() {
    ...
    return html`
      ...
      <div class="nexio-page-dragging-area" style=${styleMap(style)}></div>
    `;
  }
...
export const pageDraggingAreaWidget = WidgetViewExtension(
  ...
  literal`${unsafeStatic(NEXIO_PAGE_DRAGGING_AREA_WIDGET)}`
);

declare global {
  interface HTMLElementTagNameMap {
    [NEXIO_PAGE_DRAGGING_AREA_WIDGET]: NexioPageDraggingAreaWidget;
  }
}
