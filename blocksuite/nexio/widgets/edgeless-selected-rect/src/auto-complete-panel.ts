import { EdgelessFrameManagerIdentifier } from '@blocksuite/nexio-block-frame';
import {
  CanvasElementType,
...
import { FontFamilyIcon } from '@blocksuite/nexio-components/icons';
import {
  mountShapeTextEditor,
  SHAPE_OVERLAY_HEIGHT,
  SHAPE_OVERLAY_WIDTH,
  ShapeComponentConfig,
} from '@blocksuite/nexio-gfx-shape';
import {
  insertEdgelessTextCommand,
  mountTextElementEditor,
} from '@blocksuite/nexio-gfx-text';
import type {
  Connection,
...
export class EdgelessAutoCompletePanel extends WithDisposable(LitElement) {
  ...
  private _addFrame() {
    ...
    const props = this.std.get(EditPropsStore).applyLastProps('nexio:frame', {
      title: new Y.Text(`Frame ${frameIndex}`),
      xywh: serializeXYWH(...xywh),
      presentationIndex: frameMgr.generatePresentationIndex(),
    });
    const id = this.crud.addBlock('nexio:frame', props, surfaceBlockModel);
  ...
  private _addNote() {
    ...
    store.addBlock('nexio:paragraph', { type: 'text' }, id);
  ...
  private _showFrameOverlay() {
    ...
    const strokeColor = this.std
      .get(ThemeProvider)
      .getCssVariableColor('--nexio-black-30');
  ...
  private _showNoteOverlay() {
    ...
    const background = this.edgeless.std
      .get(ThemeProvider)
      .getColorValue(
        this.edgeless.std.get(EditPropsStore).lastProps$.value['nexio:note']
          .background,
