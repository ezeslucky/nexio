import { notifyLinkedDocSwitchedToEmbed } from '@blocksuite/nexio-components/notification';
import {
  ActionPlacement,
...
import { NexioReference } from '../reference-node';
  ...
  actions: [
    {
      ...
      content(ctx) {
        ...
        return html`<nexio-linked-doc-title
          .title=${originalTitle}
          .open=${open}
        ></nexio-linked-doc-title>`;
      },
    },
    {
      id: 'c.conversions',
      actions: [
        ...
        },
        {
          ...
          disabled(ctx) {
            const target = ctx.message$.peek()?.element;
            if (!(target instanceof NexioReference)) return true;
            ...
            if (target.closest('nexio-embed-synced-doc-block')) return true;
      ...
      content(ctx) {
        ...
        return html`${keyed(
          target,
          html`<nexio-view-dropdown-menu
            .actions=${actions}
            .context=${ctx}
            .onToggle=${onToggle}
            .viewType$=${viewType$}
          ></nexio-view-dropdown-menu>`
        )}`;
      },
      when(ctx) {
        const target = ctx.message$.peek()?.element;
        if (!(target instanceof NexioReference)) return false;
        if (!target.block) return false;

        if (ctx.flags.isNative()) return false;
        if (
          target.block.closest('nexio-database') ||
          target.block.closest('nexio-table')
        )
          return false;
