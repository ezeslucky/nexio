...
import type { ReferenceInfo } from '@blocksuite/nexio-model';
import {
  DEFAULT_DOC_NAME,
  REFERENCE_NODE,
...
import { nexioTextStyles } from '@blocksuite/nexio-shared/styles';
...
export class NexioReference extends WithDisposable(ShadowlessElement) {
  static override styles = css`
    ...
    .nexio-reference:hover {
      background: var(--nexio-hover-color);
    }

    .nexio-reference[data-selected='true'] {
      background: var(--nexio-hover-color);
    }

    .nexio-reference-title {
      margin-left: 4px;
      border-bottom: 0.5px solid var(--nexio-divider-color);
      transition: border 0.2s ease-out;
    }
    .nexio-reference-title:hover {
      border-bottom: 0.5px solid var(--nexio-icon-color);
    }
  `;
  ...
  get inlineEditor() {
    const inlineRoot = this.closest<InlineRootElement<NexioTextAttributes>>(
      `[${INLINE_ROOT_ATTR}]`
    );
  ...
  override render() {
    ...
    const style = nexioTextStyles(
      attributes,
      isDeleted
        ? {
            color: 'var(--nexio-text-disable-color)',
            textDecoration: 'line-through',
            fill: 'var(--nexio-text-disable-color)',
          }
        : {}
    );
