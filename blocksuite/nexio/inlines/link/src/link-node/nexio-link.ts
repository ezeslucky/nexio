...
import { normalizeUrl } from '@blocksuite/nexio-shared/utils';
...
export class NexioLink extends WithDisposable(ShadowlessElement) {
  static override styles = css`
    nexio-link a:hover [data-v-text='true'] {
  ...
  // see https://github.com/ezeslucky/nexio/issues/1540
  ...
  override render() {
    ...
    if (this.delta.attributes && this.delta.attributes?.code) {
      const codeStyle = nexioTextStyles(this.delta.attributes);
    ...
    const style = this.delta.attributes
      ? nexioTextStyles(this.delta.attributes, linkStyle)
      : {};
