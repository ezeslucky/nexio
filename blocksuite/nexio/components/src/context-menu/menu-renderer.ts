  ...
  implements MenuComponentInterface
{
  ...
  renderSearch() {
    ...
    return html` <div style=${searchStyle} class="nexio-menu-search-container">
      <div
        style="font-size:20px;display:flex;align-items:center;color: var(--nexio-text-secondary-color)"
      >
        ${SearchIcon()}
      </div>
      <input
        autocomplete="off"
        class="nexio-menu-search"
    ...
    return html`
      ...
      >
        ${title.onBack
          ...
          : nothing}
        <div
          style="flex:1;font-weight:500;font-size: 14px;line-height: 22px;color: var(--nexio-text-primary-color)"
        >
          ${title.text}
        </div>
        ${title.postfix?.()}
...
export const createModal = (container: HTMLElement = document.body) => {
  ...
  div.style.fontFamily = 'var(--nexio-font-family)';
