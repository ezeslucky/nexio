...
import { isStrictUrl } from '@blocksuite/nexio-shared/utils';
import type {
  BeforeinputHookCtx,
...
const autoIdentifyLink = (ctx: HookContext<NexioTextAttributes>) => {
...
function handleExtendedAttributes(
  ctx:
    | BeforeinputHookCtx<NexioTextAttributes>
    | CompositionEndHookCtx<NexioTextAttributes>
) {
...
export const onVBeforeinput = (
  ctx: BeforeinputHookCtx<NexioTextAttributes>
) => {
...
export const onVCompositionEnd = (
  ctx: CompositionEndHookCtx<NexioTextAttributes>
) => {
  handleExtendedAttributes(ctx);
};
