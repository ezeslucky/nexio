...
import { focusTextModel, type RichText } from '@blocksuite/nexio-rich-text';
...
export class DocTitle extends WithDisposable(ShadowlessElement) {
  ...
  private _getOrCreateFirstPageVisibleNote() {
    ...
    const noteId = this.doc.addBlock('nexio:note', {}, this._rootModel, 0);
  ...
  private get _viewport() {
    return (
      this.closest<HTMLElement>('.nexio-page-viewport') ??
      this.closest<HTMLElement>('.nexio-edgeless-viewport')
    );
  }
