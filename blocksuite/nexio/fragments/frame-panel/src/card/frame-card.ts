...
import { on, once } from '@blocksuite/nexio-shared/utils';
...
const styles = css`
  ...
  .frame-card-container.dragging {
    ...
    top: 0;
    left: 0;
    z-index: calc(var(--nexio-z-index-popover, 0) + 3);
  }
...
export const NEXIO_FRAME_CARD = 'nexio-frame-card';
...
export class FrameCard extends WithDisposable(ShadowlessElement) {
  ...
  override render() {
    ...
    >
      ${this.status === 'dragging'
        ? nothing
        : html`<nexio-frame-card-title
            .cardIndex=${this.cardIndex}
            .frame=${this.frame}
          ></nexio-frame-card-title>`}
      <div
