import { EdgelessFrameManager } from '@blocksuite/nexio-block-frame';
...
const styles = css`
  ...
  .no-frame-placeholder {
    ...
    width: 230px;
    height: 48px;
    color: var(--nexio-text-secondary-color, #8e8d91);
    text-align: center;
  ...
  .insert-indicator {
    height: 2px;
    border-radius: 1px;
    background-color: var(--nexio-blue-600);
    position: absolute;
    contain: layout size;
    width: 284px;
    left: 0;
...
export const NEXIO_FRAME_PANEL_BODY = 'nexio-frame-panel-body';
  ...
  private _setDocDisposables(doc: Store) {
    ...
    this._docDisposables.add(
      doc.slots.blockUpdated.subscribe(({ type, flavour }) => {
        if (flavour === 'nexio:frame' && type !== 'update') {
...
declare global {
  interface HTMLElementTagNameMap {
    [NEXIO_FRAME_PANEL_BODY]: FramePanelBody;
  }
}
