...
import { FileSizeLimitProvider } from '@blocksuite/nexio-shared/services';
import {
  readImageSize,
  transformModel,
  withTempBlobData,
...
export const AttachmentEmbedConfigIdentifier =
  createIdentifier<AttachmentEmbedConfig>(
    'NexioAttachmentEmbedConfigIdentifier'
  );
...
const embedConfig: AttachmentEmbedConfig[] = [
  {
    ...
    check: model =>
      model.store.schema.flavourSchemaMap.has('nexio:image') &&
    ...
    render: (_, blobUrl) => {
      ...
      return html`
        <iframe
          ...
          credentialless
        ></iframe>
        <div class="nexio-attachment-embed-event-mask"></div>
      `;
    },
  },
  {
...
async function turnIntoImageBlock(model: AttachmentBlockModel) {
  if (!model.store.schema.flavourSchemaMap.has('nexio:image')) {
  ...
  transformModel(model, 'nexio:image', imageProp);
}
