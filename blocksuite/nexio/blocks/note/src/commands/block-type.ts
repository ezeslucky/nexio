...
> = (ctx, next) => {
  ...
  const appendDivider: Command<{}, { updatedBlocks: BlockModel[] }> = (
    ...
    const id = doc.addBlock('nexio:divider', {}, parent, index + 1);
    if (!nextSibling) {
      nextSiblingId = doc.addBlock('nexio:paragraph', {}, parent);
    }
  ...
  const [result, resultCtx] = std.command
    ...
    .try(chain => [
      chain.pipe((_, next) => {
        if (['nexio:code', 'nexio:divider'].includes(flavour)) {
          return next();
        }
        return false;
      }),
