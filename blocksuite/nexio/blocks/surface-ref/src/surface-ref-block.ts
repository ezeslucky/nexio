import { FrameBlockComponent } from '@blocksuite/nexio-block-frame';
import {
  EdgelessCRUDIdentifier,
  getSurfaceBlock,
} from '@blocksuite/nexio-block-surface';
import type { BlockCaptionEditor } from '@blocksuite/nexio-components/caption';
import { whenHover } from '@blocksuite/nexio-components/hover';
...
import { requestConnectedFrame } from '@blocksuite/nexio-shared/utils';
...
export class SurfaceRefBlockComponent extends BlockComponent<SurfaceRefBlockModel> {
  static override styles = css`
    ...
    nexio-surface-ref:not(:hover)
      nexio-surface-ref-toolbar:not([data-open-menu-display='show']) {
      display: none;
    }
    ...
    .nexio-surface-ref.focused {
      border-color: ${unsafeCSSVarV2('edgeless/frame/border/active')};
    }

    .nexio-surface-ref.comment-highlighted {
  ...
  private get _shouldRender() {
    return (
      ...
      !this.parentComponent?.closest('nexio-surface-ref')
    );
  }
  ...
  @query('.nexio-surface-ref')
  accessor hoverableContainer!: HTMLDivElement;

  @query('nexio-surface-ref > block-caption-editor')
...
declare global {
  interface HTMLElementTagNameMap {
    'nexio-surface-ref': SurfaceRefBlockComponent;
  }
}
