...
import { SurfaceRefBlockSchema } from '@blocksuite/nexio-model';
...
export class SurfaceRefViewExtension extends ViewExtensionProvider {
  ...
  override setup(context: ViewExtensionContext): void {
    ...
    if (isEdgeless) {
      context.register([
        BlockViewExtension(flavour, literal`nexio-edgeless-surface-ref`),
      ]);
    } else {
      context.register([
        BlockViewExtension(flavour, literal`nexio-surface-ref`),
