...
import { DefaultInlineManagerExtension } from '@blocksuite/nexio-inline-preset';
import {
  type CalloutBlockModel,
  ParagraphBlockModel,
} from '@blocksuite/nexio-model';
import { focusTextModel } from '@blocksuite/nexio-rich-text';
import { EDGELESS_TOP_CONTENTEDITABLE_SELECTOR } from '@blocksuite/nexio-shared/consts';
import {
  DocModeProvider,
  type IconData,
...
import type { UniComponent } from '@blocksuite/nexio-shared/types';
...
// Copy of renderUniLit and UniLit from nexio-data-view
...
export class CalloutBlockComponent extends CaptionedBlockComponent<CalloutBlockModel> {
  ...
  private _getEmojiMarginTop(): string {
    ...
    const marginTopMap: Record<string, string> = {
      'nexio:paragraph:h1': '23px',
      'nexio:paragraph:h2': '20px',
      'nexio:paragraph:h3': '16px',
      'nexio:paragraph:h4': '15px',
      'nexio:paragraph:h5': '14px',
      'nexio:paragraph:h6': '13px',
    };

    // For heading blocks, use the type to determine margin
    if (flavour === 'nexio:paragraph') {
  ...
  private _toggleIconPicker(event: MouseEvent) {
    ...
    wrapper.style.boxShadow = 'var(--nexio-menu-shadow)';
  ...
  private readonly _handleBlockClick = (event: MouseEvent) => {
    ...
    if (
      target.closest('.nexio-callout-emoji-container') ||
      target.classList.contains('nexio-callout-emoji')
    ) {
      return;
    }
    ...
    const paragraphId = this.store.addBlock('nexio:paragraph', {}, this.model);
