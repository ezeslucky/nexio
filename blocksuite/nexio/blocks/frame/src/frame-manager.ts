import type { SurfaceBlockModel } from '@blocksuite/nexio-block-surface';
import { Overlay } from '@blocksuite/nexio-block-surface';
...
export class EdgelessFrameManager extends GfxExtension {
  ...
  private _addFrameBlock(bound: Bound) {
    ...
    const id = this.gfx.doc.addBlock('nexio:frame', props, surfaceModel);
...
export function isFrameBlock(element: unknown): element is FrameBlockModel {
  return !!element && (element as BlockModel).flavour === 'nexio:frame';
}
