import { getEmbedCardIcons } from '@blocksuite/nexio-block-embed';
import { LoadingIcon, WebIcon16 } from '@blocksuite/nexio-components/icons';
import { ImageProxyService } from '@blocksuite/nexio-shared/adapters';
import { ThemeProvider } from '@blocksuite/nexio-shared/services';
import { getHostName } from '@blocksuite/nexio-shared/utils';
...
export class BookmarkCard extends SignalWatcher(
  ...
  override render() {
    ...
    return html`
      <div
        class="nexio-bookmark-card ${cardClassMap}"
        @click=${this.bookmark.handleClick}
        @dblclick=${this.bookmark.handleDoubleClick}
      >
        <div class="nexio-bookmark-content">
          <div class="nexio-bookmark-content-title">
            <div class="nexio-bookmark-content-title-icon">${titleIcon}</div>
            <div class="nexio-bookmark-content-title-text">${titleText}</div>
          </div>
          <div class="nexio-bookmark-content-description">
            ${descriptionText}
          </div>
          <div class="nexio-bookmark-content-url-wrapper">
            <div
              class="nexio-bookmark-content-url"
              @click=${this.bookmark.open}
            >
              <span>${getHostName(url)}</span>
              <div class="nexio-bookmark-content-url-icon">
                ${OpenInNewIcon({ width: '12', height: '12' })}
              </div>
            </div>
          </div>
        </div>
        <div class="nexio-bookmark-banner">${bannerImage}</div>
      </div>
    `;
  }
