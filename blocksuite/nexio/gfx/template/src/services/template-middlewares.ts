import { generateElementId, sortIndex } from '@blocksuite/nexio-block-surface';
import type { ConnectorElementModel } from '@blocksuite/nexio-model';
...
export const replaceIdMiddleware = (job: TemplateJob) => {
  ...
  const regenerateBlockId = (data: SlotBlockPayload['data']) => {
    ...
    if (blockJson.flavour === 'nexio:surface-ref') {
      assertType<
        SnapshotNode<{
          reference: string;
        }>
    ...
    if (blockJson.flavour === 'nexio:surface') {
    ...
    if (blockJson.flavour === 'nexio:frame') {
...
export const createStickerMiddleware = (
  ...
  return (job: TemplateJob) => {
    ...
    const changeInserPosition = (blockJson: BlockSnapshot) => {
      if (blockJson.flavour === 'nexio:image' && blockJson.props.xywh) {
        const bound = Bound.deserialize(blockJson.props['xywh'] as string);
