...
import type { GroupElementModel } from '@blocksuite/nexio-model';
...
export const group: ElementRenderer<GroupElementModel> = (
  ...
  if (elements.includes(model.id)) {
    if (model.showTitle) {
      ...
      ctx.strokeStyle = renderer.getPropertyValue('--nexio-blue');
...
function renderTitle(
  ...
  ctx.font = font;
  ctx.fillStyle = renderer.getPropertyValue('--nexio-blue');
  ctx.textAlign = 'left';
