import { DomElementRendererExtension } from '@blocksuite/nexio-block-surface';
import { FontWeight, type GroupElementModel } from '@blocksuite/nexio-model';
...
export const GroupDomRendererExtension = DomElementRendererExtension(
  ...
  (model: GroupElementModel, domElement, renderer) => {
    ...
    const renderTitle = () => {
      ...
      titleElement.style.color = renderer.getPropertyValue('--nexio-blue');
    ...
    if (elements.includes(model.id)) {
      if (model.showTitle) {
        ...
        domElement.style.outlineColor =
          renderer.getPropertyValue('--nexio-blue');
