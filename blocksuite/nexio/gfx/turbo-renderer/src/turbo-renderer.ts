 ...
 *   a.  Add `@blocksuite/nexio/gfx/turbo-renderer` as a dependency in `package.json` and create a `src/turbo` directory.
...
export class ViewportTurboRendererExtension extends GfxExtension {
  ...
  override mounted() {
    const mountPoint = document.querySelector('.nexio-edgeless-viewport');
    if (mountPoint) {
