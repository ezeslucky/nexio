...
export const synced: InitFn = (collection: Workspace, id: string) => {
  ...
  docSyncedPage.load(() => {
    ...
    const rootId = docSyncedPage.addBlock('nexio:page', {
      title: new Text('Synced - Page View'),
    });

    docSyncedPage.addBlock('nexio:surface', {}, rootId);

    // Add note block inside root block
    const noteId = docSyncedPage.addBlock('nexio:note', {}, rootId);
  ...
  docSyncedEdgeless.load(() => {
    // Add root block and surface block at root level
    const rootId = docSyncedEdgeless.addBlock('nexio:page', {
      title: new Text('Synced - Edgeless View'),
    });

    docSyncedEdgeless.addBlock('nexio:surface', {}, rootId);

    // Add note block inside root block
    const noteId = docSyncedEdgeless.addBlock('nexio:note', {}, rootId);
  ...
  docMain.load(() => {
    // Add root block and surface block at root level
    const rootId = docMain.addBlock('nexio:page', {
      title: new Text('Home doc, having synced blocks'),
    });

    const surfaceId = docMain.addBlock('nexio:surface', {}, rootId);
    const noteId = docMain.addBlock('nexio:note', {}, rootId);
