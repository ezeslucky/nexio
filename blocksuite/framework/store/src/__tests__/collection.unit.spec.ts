...
describe('basic', () => {
  ...
  it('doc ready lifecycle', () => {
    ...
    store.load(() => {
      ...
      store.addBlock('nexio:note', {}, rootId);
    });
...
describe('addBlock', () => {
  ...
  it('can add model with props', () => {
    const doc = createTestDoc();
    doc.addBlock('nexio:page', { title: new Text('hello') });
  ...
  it('can add multi models', () => {
    ...
    doc.addBlocks(
      [
        { flavour: 'nexio:paragraph', blockProps: { type: 'h1' } },
        { flavour: 'nexio:paragraph', blockProps: { type: 'h2' } },
      ],
      noteId
    );
  ...
  it('can observe slot events', async () => {
    ...
    assert.equal(block.flavour, 'nexio:page');
  });
  ...
  it('can add block to root', async () => {
    ...
    queueMicrotask(() => {
      const rootId = doc.addBlock('nexio:page');
      noteId = doc.addBlock('nexio:note', {}, rootId);
    });
    await waitOnce(doc.slots.rootAdded);
    const { root } = doc;
    if (!root) throw new Error('root is null');

    assert.equal(root.flavour, 'nexio:page');

    doc.addBlock('nexio:paragraph', {}, noteId!);
    assert.equal(root.children[0].flavour, 'nexio:note');
    assert.equal(root.children[0].children[0].flavour, 'nexio:paragraph');
...
describe('deleteBlock', () => {
  ...
  it('bring children to parent', () => {
    ...
    const rootId = doc.addBlock('nexio:page', {});
    const noteId = doc.addBlock('nexio:note', {}, rootId);
    const p1 = doc.addBlock('nexio:paragraph', {}, noteId);
    doc.addBlock('nexio:paragraph', {}, p1);
    doc.addBlock('nexio:paragraph', {}, p1);
  ...
  it('bring children to other block', () => {
    ...
    const noteId = doc.addBlock('nexio:note', {}, rootId);
    const p1 = doc.addBlock('nexio:paragraph', {}, noteId);
    const p2 = doc.addBlock('nexio:paragraph', {}, noteId);
    doc.addBlock('nexio:paragraph', {}, p1);
    doc.addBlock('nexio:paragraph', {}, p1);
    doc.addBlock('nexio:paragraph', {}, p2);
  ...
  it('can delete model with parent', () => {
    ...
    const noteId = doc.addBlock('nexio:note', {}, rootModel.id);
...
describe('getBlock', () => {
  it('can get block by id', () => {
    ...
    assert.equal(text.flavour, 'nexio:paragraph');
